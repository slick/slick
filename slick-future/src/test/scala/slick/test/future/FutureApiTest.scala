...
/** Tests for [[slick.future.Database]] and the `slick.future` package enrichments.
  *
  * Covers:
  ...
  */
class FutureApiTest extends FunSuite {
  ...
  private def withDb[A](testName: String)(f: slick.future.Database => A): A = {
    val db = slick.future.Database.forURL(
  ...
  test("DBIO.from accepts a Future[R] after import slick.future.*") {
  ...
  test("Database.wrap produces a working Database whose close() is a no-op") {
    ...
    val db = slick.future.Database.wrap(underlyingDb, dispatcher)
  ...
  private def freePermits(db: slick.future.Database): Long =
