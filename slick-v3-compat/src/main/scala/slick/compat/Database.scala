...
/** A Slick database that runs actions as `scala.concurrent.Future` rather than
  * `cats.effect.IO`. Intended as a drop-in replacement for the Slick 3 `Database` type for
  * Play / Akka / Pekko applications that use `Future` at their framework boundary and want to
  * migrate to Slick 4 with minimal changes.
  *
  ...
  * `slick.compat.Database`.
  *
  * @see [[slick.compat]] for the `DBIO.from(future)` enrichment and the
  *      `DatabasePublisher[T]` type alias.
  */
final class Database private[compat] (
...
object Database {
  ...
  /** Wrap an existing `Database[IO]` and let compat own the publisher dispatcher lifecycle.
    *
  ...
  // ---------------------------------------------------------------------------
  // Factory methods — slick-v3-compat owns lifecycle
  // ---------------------------------------------------------------------------
