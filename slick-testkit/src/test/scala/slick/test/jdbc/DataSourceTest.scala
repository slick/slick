...
class DataSourceTest {
  ...
  @Test def testMaxConnectionsFallbackToNumThreads: Unit = {
    ...
    try {
      assertEquals("maxConnections should fallback to numThreads", Some(10), db.source.maxConnections)
      assertEquals("default queueSize should be 1000", 1000L, db.availableAdmissionQueueSlots.unsafeRunSync())
      assertEquals("default maxInflightActions should be 2 * maxConnections", 20L, db.availableInflightSlots.unsafeRunSync())
    } finally close.unsafeRunSync()
  }
  ...
  @Test def testMaxConnectionsPreferredOverNumThreads: Unit = {
    ...
    try {
      ...
      assertEquals("default queueSize should be 1000", 1000L, db.availableAdmissionQueueSlots.unsafeRunSync())
      assertEquals("default maxInflightActions should be 2 * maxConnections", 24L, db.availableInflightSlots.unsafeRunSync())
    } finally close.unsafeRunSync()
  }
  ...
  @Test def testInflightAndQueueConfig: Unit = {
    ...
    try {
      ...
      assertEquals("maxInflightActions must be at least maxConnections", 10L, db.availableInflightSlots.unsafeRunSync())
    } finally close.unsafeRunSync()
  }
