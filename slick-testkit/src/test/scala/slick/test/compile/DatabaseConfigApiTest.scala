...
/** Compile-time test: verifies that the DatabaseConfig factory API produces a
  ...
  * Users should use cats.Database.resource(dc).use(...) or P.backend.makeDatabase[IO](config)
  * for profile-precise types on 2.12. The cats.Database.resource pattern works uniformly.
  */
object DatabaseConfigApiTest {
  ...
  // forConfig — cats.Database.resource.use pattern works on all Scala versions
  locally {
    val dc = DatabaseConfig.forConfig[H2Profile]("mydb")
    val _result: IO[Int] = cats.Database.resource(dc).use(db => db.run(DBIO.successful(1)))
  }

  // forProfileConfig with a stable profile val — cats.Database.resource.use pattern
  locally {
    val dc = DatabaseConfig.forProfileConfig(P, "mydb")
    val _result: IO[Int] = cats.Database.resource(dc).use(db => db.run(DBIO.successful(1)))
  }

  // forURL with a stable profile val — cats.Database.resource.use pattern
  locally {
    val dc = DatabaseConfig.forURL(P, "jdbc:h2:mem:test")
    val _result: IO[Int] = cats.Database.resource(dc).use(db => db.run(DBIO.successful(1)))
  }

  // forDataSource — cats.Database.resource.use pattern
  locally {
  ...
  // forDriver — cats.Database.resource.use pattern
  locally {
