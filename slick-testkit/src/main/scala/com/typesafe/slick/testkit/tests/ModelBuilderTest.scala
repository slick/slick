...
class ModelBuilderTest extends AsyncTest[JdbcTestDB] {
  ...
  class DefaultTest(tag: Tag)
    ...
    def someStringOptionDefaultEmpty =
      column[Option[String]]("str_option_default_empty", O.Default(Some("")), O.Length(254))
    ...
    def someStringOptionDefaultNonEmpty =
      column[Option[String]]("str_option_default_non_empty", O.Default(Some("foo")), O.Length(254))
    def * = (
      someBool,
      someBoolDefaultTrue,
      someBoolDefaultFalse,
      someBoolOption,
      someBoolOptionDefaultSome,
      someBoolOptionDefaultNone,
      someString,
      someStringDefaultNonEmpty,
      someStringDefaultEmpty,
      someStringOption,
      someStringOptionDefaultEmpty,
      someStringOptionDefaultNonEmpty,
      someStringOptionDefaultNone
    )
  }
  ...
  def test = ifCap(jcap.createModel) {
    ...
    for {
      ...
      _ <- createModel(Some(tables.filter(_.name.name.toUpperCase == "POSTS")), ignoreInvalidDefaults = false).map {
        model =>
          model.assertConsistency()
          assertEquals(0, model.tables.map(_.foreignKeys.size).sum)
      }
      _ <- createModel(Some(tables.filter(_.name.name.toUpperCase == "CATEGORIES")), ignoreInvalidDefaults = false)
      ...
      _ = {
        ...
        assertEquals(
          List("id"),
          categories.columns.filter(_.options.exists(_ == ColumnOption.PrimaryKey)).map(_.name).toList
        )
        assertEquals(
          (123, true),
          categories.columns
            ...
            .head
            .options
            .collect { case RelationalProfile.ColumnOption.Length(length, varying) =>
              (length, varying)
            }
            .head
        )
        ...
        categories.columns.foreach {
          _.options.foreach {
            case RelationalProfile.ColumnOption.Length(length, _) => length < 256
      ...
      _ = {
        ...
        def tpe(col: String) = posts.columns
          .filter(_.name == col)
          .head
          .options
          .collect { case SqlProfile.ColumnOption.SqlType(tpe) =>
            tpe
          }
          .head
        assert(
      ...
      _ = {
        ...
        ifCapU(jcap.defaultValueMetaData) {
          ...
          def columnDefault(name: String) = column(name).options.collect {
