...
class JdbcMapperTest extends AsyncTest[JdbcTestDB] {
  ...
  def testHListShapeMismatchCompilationHang = {
    ...
    // Case class with 25 fields to trigger HList path
    ...
    // This test should fail compilation with a clear error message, not hang
    // When the fix is properly implemented, it should give an error like:
    // "Source and target product decomposition do not match"
    val largeTable = TableQuery[LargeTable]
    
    // The test itself doesn't need to run - the important part is that it compiles
    // with a proper error message instead of hanging
    largeTable.schema.create
  }
