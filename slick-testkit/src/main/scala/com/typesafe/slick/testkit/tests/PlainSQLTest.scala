...
class PlainSQLTest extends AsyncTest[JdbcTestDB] {
  ...
  def testInterpolation = ifCap(tcap.plainSql) {
    ...
    def userForIdAndName(id: Int, name: String) = sql"select id, name from USERS where id = $id and name = $name"
      .as[User]
    ...
    seq(
      ...
      userForIdAndName(2, "guest").head shouldYield User(
        2,
        "guest"
      ...
      userForIdAndName(
        2,
        "foo"
      ).headOption shouldYield None // TODO Support `head` and `headOption` in Plain SQL Actions
    )
  }
