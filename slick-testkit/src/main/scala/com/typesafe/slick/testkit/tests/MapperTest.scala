...
class MapperTest extends TestkitTest[JdbcTestDB] {
  ...
  def testCustomShape {
    ...
    final class PairShape[M <: Pair[_,_], U <: Pair[_,_], P <: Pair[_,_]](val shapes: Seq[Shape[_, _, _]]) extends MappedScalaProductShape[Pair[_,_], M, U, P] {
      def buildValue(elems: IndexedSeq[Any]) = Pair(elems(0), elems(1))
      def copy(shapes: Seq[Shape[_, _, _]]) = new PairShape(shapes)
    }
    implicit def pairShape[M1, M2, U1, U2, P1, P2](implicit s1: Shape[M1, U1, P1], s2: Shape[M2, U2, P2]) =
