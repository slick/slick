    ...
    if (!tdb.isEnabled)
      ...
      try {
        val is = children.iterator
          .map(ch =>
            (ch, ch.cl.getConstructor().newInstance().asInstanceOf[AsyncTest[_ >: Null <: TestDB]])
          ) // TODO why does Dotty require this cast?
    ...
    if (found != exp)
      throw cs.symbolNamer.use(
        new SlickTreeException(
          ...
          cs.tree,
          mark = _.isInstanceOf[Comprehension.Base],
...
abstract class AsyncTest[TDB >: Null <: TestDB](implicit TdbClass: ClassTag[TDB]) extends GenericTest[TDB] {
  ...
  object GetSession
      extends SynchronousDatabaseAction[
        ...
        NoStream,
        tdb.profile.backend.Context,
        tdb.profile.backend.StreamingContext,
        Effect
      ] {
  ...
  /**
   * Asynchronously consume a Reactive Stream and materialize it as a Vector, requesting new elements one by one and
   * transforming them after the specified delay. This ensures that the transformation does not run in the synchronous
  ...
  implicit class AssertionExtensionMethods[T](v: T) {
    ...
    private[this] def fixStack(f: => Unit): Unit = try f
    catch {
  ...
  implicit class CollectionDBIOActionExtensionMethods[T, +S <: NoStream, -E <: Effect](
      action: DBIOAction[Vector[T], S, E]
  ) {
