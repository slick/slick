...
class MainTest extends AsyncTest[JdbcTestDB] { mainTest =>
  ...
  def test = {
    ...
    val q1b =
      for (u <- users)
        yield (u.id, u.first.?, u.last, Case If u.id < 3 Then "low" If u.id < 6 Then "medium" Else "high")
    ...
    materialize(p1.mapResult { case (id, f, l) => User(id, f, l.orNull) })
      .flatMap { allUsers =>
        ...
        db.run(for {
          ...
          _ = r1b shouldBe expectedUserTuples.map { case (id, f, l) =>
            (id, Some(f), l, if (id < 3) "low" else if (id < 6) "medium" else "high")
          }
      ...
      .flatMap { allUsers =>
        ...
        val ordersInserts =
          for {
            u <- allUsers if u.first != "Apu" && u.first != "Snowball"
            i <- 1 to 2
          } yield orders.map(o => (o.userID, o.product, o.shipped, o.rebate)) += (u.id, "Gizmo " + ((scala.math
            .random() * 10) + 1).toInt, i == 2, Some(u.first == "Marge"))
