 ...
 *
 * This is generated using Derby currently because Derby strips column size of some columns, which works with all
 * backends. If the code was generated using model data where the size is included it would fail in derby and hsqldb.
 * The code is tested using all enabled profiles. We should also diversify generation as well at some point.
 */
object GenerateRoundtripSources {
  def main(args: Array[String]): Unit = {
    ...
    val ((gen, gen2), gen3) =
      try Await.result(db.run(ddl.create >> ((a1 zip a2) zip a3)), Duration.Inf)
      finally db.close
