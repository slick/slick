...
trait OutputHelpers {
  ...
  /**
   * The generated code stored in a map that associates the scala filename with the generated code (this map contains
   * one entry per table).
   */
  ...
  /**
   * Generates code and writes it to a file. Creates a folder structure for the given package inside the given srcFolder
  ...
  /**
   * Generates code and writes it to multiple files. Creates a folder structure for the given package inside the given
   * srcFolder and places the new files inside or overrides the existing one.
   * @group Output
   * @param folder
  ...
  def writeToMultipleFiles(profile: String, folder: String, pkg: String, container: String = "Tables"): Unit = {
    ...
    codePerTable.foreach { case (tableName, tableCode) =>
      writeStringToFile(
        packageTableCode(tableName, tableCode, pkg, container),
        folder,
        pkg,
        handleQuotedNamed(tableName) + ".scala"
      )
    }
  }
