...
abstract class AbstractSourceCodeGenerator(model: m.Model)
  ...
  def foreignKeysPerTable: Map[String, List[String]] = tables
    .map(t => t.TableValue.name -> t.model.foreignKeys.map(k => tableName(k.referencedTable.table)).toList)
    .toMap
  ...
  abstract class AbstractSourceCodeTableDef(model: m.Table) extends AbstractTableDef(model) {
    ...
    def factory =
      if (columns.size == 1 || isMappedToHugeClass) TableClass.elementType
      else s"(${TableClass.elementType}.apply _).tupled"
    ...
    trait AbstractSourceCodeTableClassDef extends AbstractTableClassDef {
      ...
      def optionFactory = {
        val accessors = columns.zipWithIndex.map { case (c, i) =>
          ...
          } else if (columns.size > 1) tuple(i)
          else "r"
