...
/**
 * Slick code generator providing the base structure and facilities. It contains a subclass as a generator for Tables,
 * which again contains subclasses for Column, etc. The implementation follows the virtual class pattern, which allows
 * flexible customization by overriding the inner classes (following the pattern).
 * @see
...
abstract class AbstractGenerator[Code, TermName, TypeName](model: m.Model)
  ...
  abstract case class AbstractTableDef(model: m.Table) { table =>
    ...
    /**
     * Database column positions in the desired user-facing order. Currently just moves the positions of AutoInc columns
     * to the end if autoIncLastAsOption is enabled.
     */
    ...
    /**
     * Default is true, i.e. a case class will be generated even if column.size > 22. Override to false to get the code
     * as before Slick 3.3, i.e. a HList based type will be generated instead.
     * @group Basic customization now overrides
     */
     ...
     * (Consider overriding EntityType.enabled instead, which affects this, too.) Disabled by default when using hlists.
     * @group Basic customization overrides
     */
    ...
    /**
     * Entity case class or type alias generator definition (Mapped case class holding a complete row of data of this
     * table).
     * @group Basic customization overrides
     */
    ...
    abstract case class AbstractColumnDef(model: m.Column) extends AbstractTermDef {
      ...
      /**
       * Underlying Scala type of this column. Override this to just affect the data type but preserve potential
       * Option-wrapping.
       * @group Basic customization overrides
       */
      ...
      /**
       * Indicates whether the exposed type of this column should be wrapped in an Option. Useful for autoInc and
       * automatically created columns. Set to autoInc to expose autoInc columns as Option.
       * @group Basic customization overrides
       */
...
trait GeneratorHelpers[Code, TermName, TypeName] {
  ...
  implicit class StringExtensions(val str: String) {
    ...
    /**
     * Capitalizes the first (16 bit) character of each word separated by one or more '_'. Lower cases all other
     * characters. Removes one '_' from each sequence of one or more subsequent '_' (to avoid collision). (Warning: Not
     * unicode-safe, uses String#apply)
     */
