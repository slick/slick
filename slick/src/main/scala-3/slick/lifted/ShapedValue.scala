...
case class ShapedValue[T, U](value: T, shape: Shape[_ <: FlatShapeLevel, T, U, _]) extends Rep[U] {
  ...
  def packedValue[R](implicit ev: Shape[_ <: FlatShapeLevel, T, _, R]): ShapedValue[R, U] =
    ShapedValue(shape.pack(value).asInstanceOf[R], shape.packedShape.asInstanceOf[Shape[FlatShapeLevel, R, U, _]])
  ...
  def <>[R: ClassTag, T](f: (U => R), g: (R => T))(implicit tt: ToTuple[T, U]) =
    new MappedProjection[R](
      shape.toNode(value),
      MappedScalaType
        .Mapper(g.andThen(tt.apply).andThen(_.get).asInstanceOf[Any => Any], f.asInstanceOf[Any => Any], None),
  ...
  def mapToExpr[R: Type, T: Type, U: Type](sv: Expr[ShapedValue[T, U]])(using Quotes): Expr[MappedProjection[R]] = {
    ...
    val (f, g, elemTpes) = Expr.summon[Mirror.ProductOf[U]] match {
      case Some(upm @ '{ $m: Mirror.ProductOf[U] { type MirroredElemTypes = elementTypes } }) =>
        ...
        val g = '{
          ((r: R) => r.asInstanceOf[Product].productIterator.foldRight(HNil: HList) { case (n, z) => new HCons(n, z) })
        ...
        utpe match {
          case utpe @ '[HList] =>
            ...
            val g = '{
              (
                  (r: R) =>
                    r.asInstanceOf[Product].productIterator.foldRight(HNil: HList) { case (n, z) => new HCons(n, z) }
              ).asInstanceOf[Any => Any]
            }
            (f, g, elemTpes)
          case _ if targetElemTpes.length == 1 =>
            val f = '{
              ((u: U) => $rpm.fromProduct(Tuple.fromArray(Array(u.asInstanceOf[AnyRef])))).asInstanceOf[Any => Any]
            }
