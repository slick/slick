...
trait SlickActionInstances {
  ...
  private type ActionMonad[Effect <: slick.dbio.Effect] = MonadError[Action[Effect], Throwable] with StackSafeMonad[Action[Effect]]

  implicit def catsMonadInstanceForSlickAction[Effect <: slick.dbio.Effect]: ActionMonad[Effect] = new MonadError[Action[Effect], Throwable] with StackSafeMonad[Action[Effect]] {
  ...
  private class CatsMonoidInstanceForSlickAction[Effect <: slick.dbio.Effect, A](using A: Monoid[A]) extends CatsSemigroupInstanceForSlickAction[Effect, A] with Monoid[Action[Effect][A]] {
