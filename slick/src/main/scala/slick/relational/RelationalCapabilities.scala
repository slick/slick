...
object RelationalCapabilities {
  ...
  /**
   * Supports the ''database'' function to get the current database name. A profile without this capability will return
   * an empty string.
   */
  val functionDatabase = Capability("relational.functionDatabase")

  /**
   * Supports the ''user'' function to get the current database user. A profile without this capability will return an
   * empty string.
   */
  ...
  /**
   * Returns only the requested number of rows even if some rows are not unique. Without this capability, non-unique
   * rows may be counted as only one row each.
   */
