...
/**
 * Create a ResultConverter for parameters and result sets. Subclasses have to provide profile-specific
 * createColumnConverter implementations.
 */
trait ResultConverterCompiler[R, W, U] {

  def compile(n: Node): ResultConverter[R, W, U, _] = n match {
    case InsertColumn(paths, fs, _) =>
      val pathConvs = paths.map { case Select(_, ElementSymbol(idx)) =>
        createColumnConverter(n, idx, Some(fs)).asInstanceOf[ResultConverter[R, W, U, Any]]
      }
  ...
  def createGetOrElseResultConverter[T](
      rc: ResultConverter[R, W, U, Option[T]],
      default: () => T
  ): ResultConverter[R, W, U, T] =
  ...
  def createTypeMappingResultConverter(
      rc: ResultConverter[R, W, U, Any],
      mapper: MappedScalaType.Mapper
  ): ResultConverter[R, W, U, Any] =
    new TypeMappingResultConverter(rc, mapper.toBase, mapper.toMapped)

  def createOptionRebuildingConverter(
      discriminator: ResultConverter[R, W, U, Boolean],
      data: ResultConverter[R, W, U, Any]
  ): ResultConverter[R, W, U, Option[Any]] =
...
final case class CompiledMapping(converter: ResultConverter[_, _, _, _], buildType: Type)
    extends NullaryNode
