...
/**
 * A profile for relational databases that does not assume the existence of SQL (or any other text-based language for
 * executing statements). It requires a relational table structure as its basic model of data.
 */
trait RelationalProfile
  ...
  trait RelationalAPI extends BasicAPI with RelationalImplicitColumnTypes {
    type FastPath[T] =
      SimpleFastPathResultConverter[ResultConverterReader, ResultConverterWriter, ResultConverterUpdater, T]
    ...
    implicit def tableQueryToTableQueryExtensionMethods[T <: RelationalProfile#Table[?], U](
        q: Query[T, U, Seq] & TableQuery[T]
    ...
    implicit def streamableCompiledInsertActionExtensionMethods[EU](
        c: StreamableCompiled[?, ?, EU]
    ): InsertActionExtensionMethods[EU] =
      createInsertActionExtensionMethods[EU](c.compiledInsert.asInstanceOf[CompiledInsert])
    implicit def queryInsertActionExtensionMethods[U, C[_]](q: Query[?, U, C]): InsertActionExtensionMethods[U] =
    ...
    implicit def fastPathExtensionMethods[T](
        mp: MappedProjection[T]
    ): FastPathExtensionMethods[ResultConverterReader, ResultConverterWriter, ResultConverterUpdater, T] =
      new FastPathExtensionMethods[ResultConverterReader, ResultConverterWriter, ResultConverterUpdater, T](mp)
  }
  ...
  class TableQueryExtensionMethods[T <: RelationalProfile#Table[?], U](val q: Query[T, U, Seq] & TableQuery[T]) {
    ...
    def findBy[P](f: T => Rep[P])(implicit
      ...
      import self.api.*
      Compiled { (p: Rep[P]) =>
        (q: Query[T, U, Seq]).filter(table => Library.==.column[Boolean](f(table).toNode, p.toNode))
      }
    }
  }

  /**
   * Run a query synchronously on the provided session. This is used by DistributedProfile until we can make it fully
   * asynchronous.
   */
  ...
  class FastPathExtensionMethods[R, W, U, T](val mp: MappedProjection[T]) {
    def fastPath(
        fpf: TypeMappingResultConverter[R, W, U, T, ?] => SimpleFastPathResultConverter[R, W, U, T]
...
object RelationalProfile {
  ...
  object ColumnOption {
    ...
    /**
     * Number of unicode characters for string-like types. Unlike DBType this is portable between different DBMS. Note
     * that for DDL Slick currently picks type CHAR when varying=false and VARCHAR when varying=true. Slick uses VARCHAR
     * or VARCHAR(254) in DDL for String columns if neither ColumnOption DBType nor Length are given.
     *
     * @param varying
...
trait RelationalTableComponent { self: RelationalProfile =>
  ...
  abstract class Table[T](_tableTag: Tag, _schemaName: Option[String], _tableName: String)
    ...
    /**
     * Note that Slick uses VARCHAR or VARCHAR(254) in DDL for String columns if neither ColumnOption DBType nor Length
     * are given.
     */
...
trait RelationalTypesComponent { self: RelationalProfile =>
  ...
  lazy val MappedColumnType: MappedColumnTypeFactory =
    null // TODO should be abstract but 2.13 doesn't allow abstract lazy vals and Dotty doesn't allow overriding a non-lazy val with a lazy val
