...
trait ResultConverter[R, W, U, T] extends Dumpable {
   ...
   *
   * @param offset
   *   The number of `PreparedStatement` parameters to skip. In a single-row operation (ex. insert into tbl values (?,
   *   ?, ?)), it should be 0. In a bulk insert operation, it should be ''C'' × ''R'', where ''C'' is the number of
   *   columns and ''R'' is the number of rows that have already been set.
   */
  ...
  /**
   * The width of this converter (in columns), corresponding to the number of columns that will be read or written by
   * it.
   */
  def width: Int
...
final case class ProductResultConverter[
    ...
    U,
    T <: Product
](elementConverters: ResultConverter[R, W, U, ?]*)
...
final case class CompoundResultConverter[R, W, U, T](width: Int, childConverters: ResultConverter[R, W, U, T]*)
    extends ResultConverter[R, W, U, T] {
  ...
  override def getDumpInfo = super.getDumpInfo.copy(children = childConverters.zipWithIndex.map { case (ch, i) =>
    (if (i == 0) "*" else "-", ch)
  })
}
...
final class GetOrElseResultConverter[R, W, U, T](child: ResultConverter[R, W, U, Option[T]], default: () => T)
    extends ResultConverter[R, W, U, T] {
...
final case class TypeMappingResultConverter[R, W, U, T, C](
    child: ResultConverter[R, W, U, C],
    toBase: T => C,
    toMapped: C => T
) extends ResultConverter[R, W, U, T] {
...
final case class OptionRebuildingResultConverter[
    ...
    U,
    T
](discriminator: ResultConverter[R, W, U, Boolean], data: ResultConverter[R, W, U, T])
    extends ResultConverter[R, W, U, Option[T]] {
...
/**
 * A `ResultConverter` that simplifies the implementation of fast path converters. It always wraps a
 * `TypeMappingResultConverter` on top of a `ProductResultConverter`, allowing direct access to the product elements.
 */
abstract class SimpleFastPathResultConverter[
    ...
    U,
    T
](protected[this] val rc: TypeMappingResultConverter[R, W, U, T, ?])
    extends ResultConverter[R, W, U, T] {
