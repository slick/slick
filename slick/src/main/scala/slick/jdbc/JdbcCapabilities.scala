...
object JdbcCapabilities {
  ...
  /**
   * Supports a native insertOrUpdate command. Otherwise the functionality is emulated on the client side. The emulation
   * uses transactions for consistency but does not guarantee atomicity, so it may fail if another insert for the same
   * key happens concurrently.
   */
