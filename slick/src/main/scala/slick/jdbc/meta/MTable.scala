...
object MTable {
  ...
  /**
   * Some DatabaseMetaData methods take arguments that are String patterns. These arguments all have names such as
   * fooPattern. Within a pattern String, "%" means match any substring of 0 or more characters, and "_" means match any
   * one character. Only metadata entries matching the search pattern are returned. If a search pattern argument is set
   * to null, that argument's criterion will be dropped from the search.
   */
  def getTables(
      cat: Option[String],
  ...
  def getTables(namePattern: String): BasicStreamingAction[Vector[MTable], MTable, Effect.Read] =
