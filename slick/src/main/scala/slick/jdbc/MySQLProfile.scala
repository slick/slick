 ...
 *
 * <ul> <li>[[slick.jdbc.JdbcCapabilities.returnInsertOther]]: When returning columns from an INSERT operation, only a
 * single column may be specified which must be the table's AutoInc column.</li>
 * <li>[[slick.sql.SqlCapabilities.sequenceLimited]]: Non-cyclic sequence may not have an upper limit.</li>
 * <li>[[slick.relational.RelationalCapabilities.joinFull]]: Full outer joins are emulated because there is not native
 * support for them.</li> <li>[[slick.jdbc.JdbcCapabilities.nullableNoDefault]]: Nullable columns always have NULL as a
 * default according to the SQL standard. Consequently MySQL treats no specifying a default value just as specifying
 * NULL and reports NULL as the default value. Some other dbms treat queries with no default as NULL default, but
 * distinguish NULL from no default value in the meta data.</li> </ul>
 *
 * Sequences are supported through an emulation which requires the schema to be created by Slick. You can also use an
 * existing schema with your own sequence emulation if you provide for each sequence ''s'' a pair of functions
 * <code>s_nextval</code> and <code>s_currval</code>.
 *
 * The default type for strings of unlimited length is "TEXT", falling back to "VARCHAR(254)" if a `Default` or
 * `PrimaryKey` column option is set. This can be changed by overriding
 * <code>slick.jdbc.MySQLProfile.defaultStringType</code> in application.conf.
 *
 * Note: Slick 3.2 also checks the old config path "slick.driver.MySQL" that was used by Slick 3.0 and 3.1, and logs a
 * warning if anything is found there. Values from the old path are *not* used anymore. This deprecation warning will be
      ...
      sym.flatMap(_.findColumnOption[RelationalProfile.ColumnOption.Length]) match {
        ...
        case None =>
          defaultStringType match {
            ...
            case None =>
              if (
                sym.flatMap(_.findColumnOption[RelationalProfile.ColumnOption.Default[_]]).isDefined ||
    ...
    override def expr(n: Node): Unit = n match {
      ...
      case Apply(Library.In, children) if children.exists(_.isInstanceOf[Union]) =>
        ...
        if (children.length == 1) {
          ...
          b.sep(children, " " + Library.In.name + " ") {
            case u @ Union(_, _, _) =>
              b"(select t.* from ("
              expr(u, skipParens)
              b") t)"
  ...
  class MySQLJdbcTypes extends JdbcTypes {
    ...
    override val instantType: InstantJdbcType = new InstantJdbcType {
      override def sqlType: Int =
        /**
         * [[Instant]] will be persisted as a [[java.sql.Types.VARCHAR]] in order to avoid losing precision, because
         * MySQL stores [[java.sql.Types.TIMESTAMP]] with second precision, while [[Instant]] stores it with a
         * millisecond one.
         */
    ...
    override val localDateTimeType: LocalDateTimeJdbcType = new LocalDateTimeJdbcType {
      override def sqlType: Int =
        /**
         * [[LocalDateTime]] will be persisted as a [[java.sql.Types.VARCHAR]] in order to avoid losing precision,
         * because MySQL stores [[java.sql.Types.TIMESTAMP]] with second precision, while [[LocalDateTime]] stores it
