...
/**
 * Specialized JDBC ResultConverter for handling values of type `Option[T]`. Boxing is avoided when the result is
 * `None`.
 */
class OptionResultConverter[T](val jdbcType: JdbcType[T], val index: Int)
  ...
  def getOrElse(default: () => T): DefaultingResultConverter[T] =
    if (jdbcType.scalaType.isPrimitive) new DefaultingResultConverter[T](jdbcType, default, index)
      ...
      new DefaultingResultConverter[T](jdbcType, default, index) {
...
/**
 * Specialized JDBC ResultConverter for handling non-`Option` values with a default. A (possibly specialized) function
 * for the default value is used to translate SQL `NULL` values.
 */
...
class IsDefinedResultConverter[T](val ti: JdbcType[T], val idx: Int)
    extends ResultConverter[ResultSet, PreparedStatement, ResultSet, Boolean] {
  def read(pr: ResultSet) = {
    ti.getValue(pr, idx)
    !ti.wasNull(pr, idx)
  }
