...
trait JdbcStatementBuilderComponent { self: JdbcProfile =>
  ...
  class JdbcCompiledInsert(source: Node) {
    ...
    def buildReturnColumns(
        node: Node
    ): (ConstArray[String], ResultConverter[ResultSet, PreparedStatement, ResultSet, _], Boolean) = {
      ...
      (
        ...
        rconv.asInstanceOf[ResultConverter[ResultSet, PreparedStatement, ResultSet, _]],
        returnOther
      )
    }
  }
  ...
  /**
   * The table name for scalar selects (e.g. "select 42 from DUAL;"), or `None` for scalar selects without a FROM clause
   * ("select 42;").
   */
        ...
        case Comprehension(sym, from: TableNode, Pure(select, _), where, None, _, None, None, None, None, false) =>
          select match {
            ...
            case ProductNode(ch) if ch.forall {
                  case Select(Ref(struct), _) if struct == sym => true; case _ => false
                } =>
  ...
  class InsertBuilder(val ins: Insert) {
    ...
    /**
     * Reorder InsertColumn indices in a mapping Node in the order of the given sequence of FieldSymbols (which may
     * contain duplicates).
     */
  ...
  class UpsertBuilder(ins: Insert) extends InsertBuilder(ins) {
    ...
    protected lazy val (pkSyms, softSyms) = syms.toSeq.partition { sym =>
      sym.options.contains(ColumnOption.PrimaryKey) || funcDefinedPKs.exists(pk =>
        pk.columns
          .collect { case Select(_, f: FieldSymbol) =>
            f
          }
    ...
    override def buildInsert: InsertBuilderResult = {
      ...
      val paramSel =
        "select " + allNames.map(n => "? as " + n).iterator.mkString(",") + scalarFrom
          .map(n => " from " + n)
          .getOrElse("")
  ...
  /**
   * Builder for SELECT statements that can be used to check for the existing of primary keys supplied to an INSERT
   * operation. Used by the insertOrUpdate emulation on databases that don't support this in a single server-side
   * statement.
   */
  ...
  /**
   * Builder for UPDATE statements used as part of an insertOrUpdate operation on databases that don't support this in a
  ...
  class TableDDLBuilder(val table: Table[_]) { self =>
    ...
    protected def addColumnList(
      ...
      for (c <- columns) c match {
        ...
        case _ =>
          throw new SlickException("Cannot use column " + c + " in " + typeInfo + " (only named columns are allowed)")
      }
    }
  }
