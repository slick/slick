...
/**
 * A `JdbcDataSource` provides a way to create a `Connection` object for a database. It is similar to a
 * `javax.sql.DataSource` but simpler. Unlike [[JdbcBackend.JdbcDatabaseDef]] it is not a part of the backend cake. This
 * trait defines the SPI for 3rd-party connection pool support.
 */
trait JdbcDataSource extends Closeable {
  ...
  /**
   * If this object represents a connection pool with a limited size, return the maximum pool size. Otherwise return
   * None. This is required to prevent deadlocks when scheduling database actions.
   */
...
object JdbcDataSource extends Logging {
  ...
  /**
   * Create a JdbcDataSource from a `Config`. See [[JdbcBackend.DatabaseFactoryDef.forConfig]] for documentation of the
...
/**
 * Create a [[JdbcDataSource]] from a `Config` object and an optional JDBC `Driver`. This is used with the
 * "connectionPool" configuration option in [[JdbcBackend.DatabaseFactoryDef.forConfig]].
 */
    ...
    val (ds, maxConnections) =
      c.getStringOpt("dataSourceClassName").orElse(c.getStringOpt("dataSourceClass")) match {
        ...
        case None =>
          ...
          BeanConfigurator
            .configure(ds, c.toProperties, Set("url", "user", "password", "properties", "driver", "driverClassName"))
          (ds, None)
      }
