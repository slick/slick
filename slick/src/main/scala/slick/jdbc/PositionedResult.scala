...
abstract class PositionedResult(val rs: ResultSet) extends Closeable { outer =>
  ...
  final def nextBigDecimal() = {
    val npos = pos + 1; val r = rs getBigDecimal npos; pos = npos; if (r eq null) null else BigDecimal(r)
  }
  ...
  final def nextBooleanOption() = {
    val npos = pos + 1; val r = rs getBoolean npos; val rr = if (rs.wasNull) None else Some(r); pos = npos; rr
  }
  final def nextBigDecimalOption() = {
    val npos = pos + 1; val r = rs getBigDecimal npos; val rr = if (rs.wasNull) None else Some(BigDecimal(r))
    pos = npos; rr
  }
  final def nextBlobOption() = {
    val npos = pos + 1; val r = rs getBlob npos; val rr = if (rs.wasNull) None else Some(r); pos = npos; rr
  }
  final def nextByteOption() = {
    val npos = pos + 1; val r = rs getByte npos; val rr = if (rs.wasNull) None else Some(r); pos = npos; rr
  }
  final def nextBytesOption() = {
    val npos = pos + 1; val r = rs getBytes npos; val rr = if (rs.wasNull) None else Some(r); pos = npos; rr
  }
  final def nextClobOption() = {
    val npos = pos + 1; val r = rs getClob npos; val rr = if (rs.wasNull) None else Some(r); pos = npos; rr
  }
  final def nextDateOption() = {
    val npos = pos + 1; val r = rs getDate npos; val rr = if (rs.wasNull) None else Some(r); pos = npos; rr
  }
  final def nextDoubleOption() = {
    val npos = pos + 1; val r = rs getDouble npos; val rr = if (rs.wasNull) None else Some(r); pos = npos; rr
  }
  final def nextFloatOption() = {
    val npos = pos + 1; val r = rs getFloat npos; val rr = if (rs.wasNull) None else Some(r); pos = npos; rr
  }
  final def nextIntOption() = {
    val npos = pos + 1; val r = rs getInt npos; val rr = if (rs.wasNull) None else Some(r); pos = npos; rr
  }
  final def nextLongOption() = {
    val npos = pos + 1; val r = rs getLong npos; val rr = if (rs.wasNull) None else Some(r); pos = npos; rr
  }
  final def nextObjectOption() = {
    val npos = pos + 1; val r = rs getObject npos; val rr = if (rs.wasNull) None else Some(r); pos = npos; rr
  }
  final def nextShortOption() = {
    val npos = pos + 1; val r = rs getShort npos; val rr = if (rs.wasNull) None else Some(r); pos = npos; rr
  }
  final def nextStringOption() = {
    val npos = pos + 1; val r = rs getString npos; val rr = if (rs.wasNull) None else Some(r); pos = npos; rr
  }
  final def nextTimeOption() = {
    val npos = pos + 1; val r = rs getTime npos; val rr = if (rs.wasNull) None else Some(r); pos = npos; rr
  }
  final def nextTimestampOption() = {
    val npos = pos + 1; val r = rs getTimestamp npos; val rr = if (rs.wasNull) None else Some(r); pos = npos; rr
  }
  ...
  final def updateBigDecimal(v: BigDecimal): Unit = {
    val npos = pos + 1; rs.updateBigDecimal(npos, v.bigDecimal); pos = npos
  }
  ...
  final def updateBooleanOption(v: Option[Boolean]): Unit = {
    val npos = pos + 1; v match { case Some(s) => rs.updateBoolean(npos, s); case None => rs.updateNull(npos) }
    pos = npos
  }
  final def updateBlobOption(v: Option[Blob]): Unit = {
    val npos = pos + 1; v match { case Some(s) => rs.updateBlob(npos, s); case None => rs.updateNull(npos) }; pos = npos
  }
  final def updateByteOption(v: Option[Byte]): Unit = {
    val npos = pos + 1; v match { case Some(s) => rs.updateByte(npos, s); case None => rs.updateNull(npos) }; pos = npos
  }
  final def updateBytesOption(v: Option[Array[Byte]]): Unit = {
    val npos = pos + 1; v match { case Some(s) => rs.updateBytes(npos, s); case None => rs.updateNull(npos) }
    pos = npos
  }
  final def updateClobOption(v: Option[Clob]): Unit = {
    val npos = pos + 1; v match { case Some(s) => rs.updateClob(npos, s); case None => rs.updateNull(npos) }; pos = npos
  }
  final def updateDateOption(v: Option[Date]): Unit = {
    val npos = pos + 1; v match { case Some(s) => rs.updateDate(npos, s); case None => rs.updateNull(npos) }; pos = npos
  }
  final def updateDoubleOption(v: Option[Double]): Unit = {
    val npos = pos + 1; v match { case Some(s) => rs.updateDouble(npos, s); case None => rs.updateNull(npos) }
    pos = npos
  }
  final def updateFloatOption(v: Option[Float]): Unit = {
    val npos = pos + 1; v match { case Some(s) => rs.updateFloat(npos, s); case None => rs.updateNull(npos) }
    pos = npos
  }
  final def updateIntOption(v: Option[Int]): Unit = {
    val npos = pos + 1; v match { case Some(s) => rs.updateInt(npos, s); case None => rs.updateNull(npos) }; pos = npos
  }
  final def updateLongOption(v: Option[Long]): Unit = {
    val npos = pos + 1; v match { case Some(s) => rs.updateLong(npos, s); case None => rs.updateNull(npos) }; pos = npos
  }
  final def updateShortOption(v: Option[Short]): Unit = {
    val npos = pos + 1; v match { case Some(s) => rs.updateShort(npos, s); case None => rs.updateNull(npos) }
    pos = npos
  }
  final def updateStringOption(v: Option[String]): Unit = {
    val npos = pos + 1; v match { case Some(s) => rs.updateString(npos, s); case None => rs.updateNull(npos) }
    pos = npos
  }
  final def updateTimeOption(v: Option[Time]): Unit = {
    val npos = pos + 1; v match { case Some(s) => rs.updateTime(npos, s); case None => rs.updateNull(npos) }; pos = npos
  }
  final def updateTimestampOption(v: Option[Timestamp]): Unit = {
    val npos = pos + 1; v match { case Some(s) => rs.updateTimestamp(npos, s); case None => rs.updateNull(npos) }
    pos = npos
  }
  final def updateBigDecimalOption(v: Option[BigDecimal]): Unit = {
    val npos = pos + 1
    v match { case Some(s) => rs.updateBigDecimal(npos, s.bigDecimal); case None => rs.updateNull(npos) }; pos = npos
  }
  final def updateObjectOption(v: Option[AnyRef]): Unit = {
    val npos = pos + 1; v match { case Some(s) => rs.updateObject(npos, s); case None => rs.updateNull(npos) }
    pos = npos
  }
  ...
  /**
   * Create an embedded PositionedResult which extends from the given dataPos column until the end of this
   * PositionedResult, starts at the current row and ends when the discriminator predicate (which can read columns
   * starting at discriminatorPos) returns false or when this PositionedResult ends.
   */
  ...
  /**
   * Create an embedded PositionedResult with a single discriminator column followed by the embedded data, starting at
   * the current position. The embedded view lasts while the discriminator stays the same. If the first discriminator
   * value is NULL, the view is empty.
   */
 ...
 */
abstract class PositionedResultIterator[+T](val pr: PositionedResult, maxRows: Int, autoClose: Boolean)
