...
trait Invoker[+R] { self =>
  ...
  /**
   * Execute the statement and return a CloseableIterator of the converted results. The iterator must either be fully
   * read or closed explicitly.
   * @param maxRows
  ...
  /**
   * Execute the statement and return the first row of the result set wrapped in Some, or None if the result set is
   * empty.
   */
  ...
  /**
   * Execute the statement and return the first row of the result set. If the result set is empty, a
  ...
  final def buildColl[C[_]](implicit
      session: JdbcBackend#JdbcSessionDef,
      canBuildFrom: Factory[R, C[R @uncheckedVariance]]
  ): C[R @uncheckedVariance] = {
