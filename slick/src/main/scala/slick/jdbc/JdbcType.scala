...
/**
 * A JdbcType object represents a Scala type that can be used as a column type in the database. Implicit JdbcTypes for
 * the standard types are provided by the profile.
 */
trait JdbcType[T] extends BaseTypedType[T] { self =>
  ...
  /**
   * Get a result column of the type. For reference types, SQL NULL values are returned as `null`, for primitive types a
   ...
   * `valueToSQLLiteral` throws a SlickException. QueryBuilder (and profile-specific subclasses thereof) uses this
   * method to treat LiteralNodes as volatile (i.e. using bind variables) as needed.
   */
