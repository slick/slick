      ...
      Stream.eval(F.ref[Option[sda.StreamState]](Some(initialState))).flatMap { liveState =>
        Stream.unfoldChunkEval[F, Option[sda.StreamState], T](Some(initialState)) {
          case None =>
            ...
            liveState.set(None) >> F.pure(None)
          ...
          case Some(st) =>
            F.blocking {
              ...
              (emitted, nextState)
            }.flatMap { case (emitted, nextState) =>
              ...
              // cleanup.
              F.uncancelable { _ =>
                liveState.set(nextState) >> F.pure(
                  ...
                  (nextState, emitted) match {
                    case (Some(s), _)    => Some((fs2.Chunk.fromOption(emitted), nextState))
          ...
          case Resource.ExitCase.Canceled =>
            ...
            liveState.get.flatMap {
              case Some(st) if st ne null =>
                F.blocking(sda.cancelStream(mkStreamingCtx(_ => ()), st)).attempt.void
              case _ =>
                F.unit
            }
          case _ =>
