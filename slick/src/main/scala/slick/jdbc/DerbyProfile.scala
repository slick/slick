 ...
 *
 * <ul> <li>[[slick.relational.RelationalCapabilities.functionDatabase]]: <code>Functions.database</code> is not
 * available in Derby. Slick will return an empty string instead.</li>
 * <li>[[slick.relational.RelationalCapabilities.replace]], [[slick.relational.RelationalCapabilities.reverse]]: These
 * String functions are not available in Derby.</li> <li>[[slick.relational.RelationalCapabilities.pagingNested]]: See
 * <a href="https://issues.apache.org/jira/browse/DERBY-5911" target="_parent">DERBY-5911</a>.</li>
 ...
 * <li>[[slick.sql.SqlCapabilities.sequenceCurr]]: <code>Sequence.curr</code> to get the current value of a sequence is
 * not supported by Derby. Trying to generate SQL code which uses this feature throws a SlickException.</li>
 * <li>[[slick.sql.SqlCapabilities.sequenceCycle]]: Sequence cycling is supported but does not conform to SQL:2008
 * semantics. Derby cycles back to the START value instead of MINVALUE or MAXVALUE.</li>
 * <li>[[slick.relational.RelationalCapabilities.zip]]: Ordered sub-queries and window functions with orderings are
 * currently not supported by Derby. These are required by <code>zip</code> and <code>zipWithIndex</code>. Trying to
 * generate SQL code which uses this feature causes the DB to throw an exception. We do not prevent these queries from
 * being generated because we expect future Derby versions to support them with the standard SQL:2003 syntax (see <a
 * href="http://wiki.apache.org/db-derby/OLAPRowNumber" target="_parent"
 * >http://wiki.apache.org/db-derby/OLAPRowNumber</a>).</li> <li>[[slick.relational.RelationalCapabilities.joinFull]]:
 * Full outer joins are emulated because there is not native support for them.</li>
 * <li>[[slick.jdbc.JdbcCapabilities.insertOrUpdate]]: InsertOrUpdate operations are emulated on the client side because
 * there is no native support for them (but there is work in progress: see <a
 * href="https://issues.apache.org/jira/browse/DERBY-3155" target="_parent" >DERBY-3155</a>).</li>
 * <li>[[slick.jdbc.JdbcCapabilities.booleanMetaData]]: Derby <= 10.6 doesn't have booleans, so Slick maps to SMALLINT
 * instead. Other jdbc drivers like MySQL map TINYINT(1) back to a Scala Boolean. Derby maps SMALLINT to an Integer and
 * that's how it shows up in the jdbc meta data, thus the original type is lost.</li>
 * <li>[[slick.jdbc.JdbcCapabilities.supportsByte]]: Derby doesn't have a corresponding type for Byte. SMALLINT is used
 * instead and mapped to Short in the Slick model.</li> <li>[[slick.relational.RelationalCapabilities.repeat]]: There's
 * not builtin string function repeat in Derby. <a href="https://db.apache.org/derby/docs/10.10/ref/rrefsqlj29026.html"
 * target="_parent" >https://db.apache.org/derby/docs/10.10/ref/rrefsqlj29026.html</a></li>
 * <li>[[slick.jdbc.JdbcCapabilities.returnMultipleInsertKey]]: Derby's driver is unable to properly return the
 * generated key for insert statements with multiple values. (<a href="https://issues.apache.org/jira/browse/DERBY-3609"
 * target="_parent"></a>DERBY-3609</li>) </ul>
 */
