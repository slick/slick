 ...
 *
 * <ul> <li>[[slick.relational.RelationalCapabilities.reverse]]: This String function is not available in DB2.</li>
 * <li>[[slick.jdbc.JdbcCapabilities.insertOrUpdate]]: InsertOrUpdate operations are emulated on the client side if
 * generated keys should be returned. Otherwise the operation is performed natively on the server side.</li>
 * <li>[[slick.jdbc.JdbcCapabilities.booleanMetaData]]: DB2 doesn't have booleans, so Slick maps to SMALLINT instead.
 * Other JDBC drivers like MySQL map TINYINT(1) back to a Scala Boolean. DB2 maps SMALLINT to an Integer and that's how
 * it shows up in the JDBC meta data, thus the original type is lost.</li>
 * <li>[[slick.jdbc.JdbcCapabilities.supportsByte]]: DB2 does not have a BYTE type.</li> </ul>
 *
 * Note: The DB2 JDBC driver has problems with quoted identifiers. Columns which are returned from inserts must not
 * require quoted names (in particular, they must not contain lower-case characters or be equal to a reserved word),
 * otherwise a bug in the DB2 JDBC driver triggers a SQL Error -206 (SQLState 42703).
 */
