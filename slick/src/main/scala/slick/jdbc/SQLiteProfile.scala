 ...
 *
 * <ul> <li>[[slick.relational.RelationalCapabilities.functionDatabase]],
 * [[slick.relational.RelationalCapabilities.functionUser]]: <code>Functions.user</code> and
 * <code>Functions.database</code> are not available in SQLite. Slick will return empty strings for both.</li>
 * <li>[[slick.relational.RelationalCapabilities.joinFull]], [[slick.relational.RelationalCapabilities.joinRight]]:
 * Right and full outer joins are emulated because there is not native support for them.</li>
 * <li>[[slick.jdbc.JdbcCapabilities.mutable]]: SQLite does not allow mutation of result sets. All cursors are
 * read-only.</li> <li>[[slick.sql.SqlCapabilities.sequence]]: Sequences are not supported by SQLite.</li>
 ...
 * <li>[[slick.relational.RelationalCapabilities.typeBigDecimal]]: SQLite does not support a decimal type.</li>
 * <li>[[slick.relational.RelationalCapabilities.typeBlob]]: Blobs are not supported by the SQLite JDBC driver (but
 * binary data in the form of <code>Array[Byte]</code> is).</li> <li>[[slick.relational.RelationalCapabilities.zip]]:
 * Row numbers (required by <code>zip</code> and <code>zipWithIndex</code>) are not supported. Trying to generate SQL
 * code which uses this feature throws a SlickException.</li> <li>[[slick.jdbc.JdbcCapabilities.insertOrUpdate]]:
 * InsertOrUpdate operations are emulated on the client side if the data to insert contains an `AutoInc` field.
 * Otherwise the operation is performed natively on the server side.</li>
 * <li>[[slick.jdbc.JdbcCapabilities.defaultValueMetaData]]: The stable xerial sqlite-jdbc driver 3.7.2 does not return
 * default values for columns in the DatabaseMetaData. Consequently they also do not appear in Slick's model. This has
 * been fixed in sqlite-jdbc, but the only released version that contains the fix is milestone 3.7.15-M1. You can use it
 * instead of the stable 3.7.2 in order to get default values with SQLite. Also see
 * https://code.google.com/p/sqlite-jdbc/issues/detail?id=27 </li> <li>[[slick.jdbc.JdbcCapabilities.booleanMetaData]]:
 * SQLite doesn't have booleans, so Slick maps to INTEGER instead. Other jdbc drivers like MySQL map TINYINT(1) back to
 * a Scala Boolean. SQLite maps INTEGER to an Integer and that's how it shows up in the jdbc meta data, thus the
 * original type is lost.</li> <li>[[slick.jdbc.JdbcCapabilities.distinguishesIntTypes]]: SQLite does not distinguish
 * integer types and maps them all to Int in the meta data.</li> <li>[[slick.jdbc.JdbcCapabilities.supportsByte]]:
 * SQLite does not distinguish integer types and maps them all to Int in the meta data.</li>
 * <li>[[slick.jdbc.JdbcCapabilities.returnMultipleInsertKey]]: SQLite returns the last generated key only.</li> </ul>
 */
trait SQLiteProfile extends JdbcProfile with JdbcActionComponent.MultipleRowsPerStatementSupport {
  ...
  class SQLiteModelBuilder(mTables: Seq[MTable], ignoreInvalidDefaults: Boolean)(implicit ec: ExecutionContext)
    ...
    class SQLiteColumnBuilder(tableBuilder: TableBuilder, meta: MColumn) extends ColumnBuilder(tableBuilder, meta) {
      ...
      override def default: Option[Option[Any]] = meta.columnDef
        .map((_, tpe))
        .collect {
          ...
          case (v, "java.sql.Timestamp") =>
            ...
            val convertors = Seq(
              ...
              (s: String) =>
                Timestamp.from(
                  LocalTime
                    .from(DateTimeFormatter.ISO_LOCAL_TIME.parse(s))
            ...
            val v2 = v.replace("\"", "")
            convertors.iterator.map(fn => Try(fn(v2))).collectFirst { case Success(v) =>
              Some(v)
            }
        }
    ...
    override def readIndices(t: MTable) = super
      .readIndices(t)
      .map(
        _.filterNot(
  ...
  private trait SQLiteInsertAll[U] extends InsertActionComposerImpl[U] {
    override def insertAll(values: Iterable[U], rowsPerStatement: RowsPerStatement = RowsPerStatement.All) =
      super.insertAll(
        values = values,
        rowsPerStatement = if (compiled.standardInsert.ibr.fields.isEmpty) {
  ...
  override def createReturningInsertActionComposer[U, QR, RU](
      compiled: JdbcCompiledInsert,
      keys: Node,
      mux: (U, QR) => RU
