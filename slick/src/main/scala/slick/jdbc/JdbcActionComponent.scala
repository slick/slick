...
trait JdbcActionComponent extends SqlActionComponent { self: JdbcProfile =>
  ...
  abstract class SimpleJdbcProfileAction[+R](_name: String, val statements: Vector[String])
      extends SynchronousDatabaseAction[
        ...
        JdbcBackend#JdbcStreamingActionContext,
        Effect
      ]
  ...
  protected object StartTransaction
      extends SynchronousDatabaseAction[
        Unit,
        NoStream,
        JdbcBackend#JdbcActionContext,
  ...
  class JdbcActionExtensionMethods[E <: Effect, R, S <: NoStream](a: DBIOAction[R, S, E]) {
    ...
    /**
     * Run this Action transactionally. This does not guarantee failures to be atomic in the presence of error handling
     * combinators. If multiple `transactionally` combinators are nested, only the outermost one will be backed by an
     * actual database transaction. Depending on the outcome of running the Action it surrounds, the transaction is
     * committed if the wrapped Action succeeds, or rolled back if the wrapped Action fails. When called on a
     * [[slick.dbio.SynchronousDatabaseAction]], this combinator gets fused into the action.
     */
     ...
     * `transactionally` Action. The semantics of using it inside a transaction are database-dependent. It does not
     * create a transaction by itself but it pins the session.
     */
    ...
    /**
     * Run this Action with the given statement parameters. Any unset parameter will use the current value. The
     ...
     * @param statementInit
     *   A function which is run on every `Statement` or `PreparedStatement` directly after creating it. This can be
     *   used to set additional statement parameters (e.g. `setQueryTimeout`). When multiple `withStatementParameters`
     *   Actions are nested, all init functions are run, starting with the outermost one.
     * @param fetchSize
    ...
    def withStatementParameters(
      ...
      new PushStatementParameters(
        JdbcBackend.StatementParameters(rsType, rsConcurrency, rsHoldability, statementInit, fetchSize)
  ...
  class MutatingResultAction[T](
      ...
      sendEndMarker: Boolean
        ...
        ResultSetMutator[T]
      ], JdbcBackend#JdbcActionContext, JdbcBackend#JdbcStreamingActionContext, Effect]
      with ProfileAction[Nothing, Streaming[ResultSetMutator[T]], Effect] { streamingAction =>
    class Mutator(val prit: PositionedResultIterator[T], val bufferNext: Boolean, val inv: QueryInvokerImpl[T])
    ...
    override def emitStream(
        ctx: JdbcBackend#JdbcStreamingActionContext,
        limit: Long,
        state: StreamState
    ): StreamState = {
      val mu =
        if (state ne null) state
          ...
          new Mutator(
            inv
              .results(0, defaultConcurrency = invokerMutateConcurrency, defaultType = invokerMutateType)(ctx.session)
    ...
    override def cancelStream(ctx: JdbcBackend#JdbcStreamingActionContext, state: StreamState): Unit = state.prit
      .close()
    ...
    def result: ProfileAction[R, S, Effect.Read] = {
      def findSql(n: Node): String = n match {
        ...
        case ParameterSwitch(cases, default) =>
          findSql(cases.find { case (f, _) => f(param) }.map(_._2).getOrElse(default))
      }
      (tree match {
        case (rsm @ ResultSetMapping(_, compiled, CompiledMapping(_, _))) :@ (ct: CollectionType) =>
          val sql = findSql(compiled)
          new StreamingInvokerAction[R, Any, Effect] { streamingAction =>
            protected[this] def createInvoker(sql: Iterable[String]): Invoker[Any] =
  ...
  class JdbcStreamingQueryActionExtensionMethodsImpl[R, T](tree: Node, param: Any)
    ...
    /**
     * Create an Action that can be streamed in order to modify a mutable result set. All stream elements will be the
     * same [[slick.jdbc.ResultSetMutator]] object but it is in a different state each time. Thre resulting stream is
     * always non-buffered and events can be processed either synchronously or asynchronously (but all processing must
     * happen in sequence).
     *
     * @param sendEndMarker
     *   If set to true, an extra event is sent after the end of the result set, poviding you with a chance to insert
     *   additional rows after seeing all results. Only `end` (to check for this special event) and `insert` may be
     *   called in the ResultSetMutator in this case.
     */
    def mutate(
  ...
  class DeleteActionExtensionMethodsImpl(tree: Node, param: Any) {
    ...
    def delete: ProfileAction[Int, NoStream, Effect.Write] = {
      ...
      new SimpleJdbcProfileAction[Int]("delete", Vector(sres.sql)) {
        def run(ctx: JdbcBackend#JdbcActionContext, sql: Vector[String]): Int = ctx.session
  ...
  def createReturningInsertActionComposer[U, QR, RU](
  ...
  trait SimpleInsertActionComposer[U] extends super.InsertActionExtensionMethodsImpl[U] {
    ...
    /**
     * The return type for `insertOrUpdate` operations. Note that the Option return value will be None if it was an
     * update and Some if it was an insert
     */
    ...
    /**
     * Insert a single row, including AutoInc columns. This is not supported by all database engines (see
     ...
     *   [[RowsPerStatement.All]] to use a single statement to insert all rows at once, or [[RowsPerStatement.One]] to
     *   use a separate SQL statement for each row. Even so, if supported this will use JDBC's batching functionality.
     * @return
     *   Some(rowsAffected), or None if the database returned no row count for some part of the batch. If any part of
     *   the batch fails, an exception is thrown.
     */
    ...
    /**
     * Insert multiple rows, skipping AutoInc columns. Uses JDBC's batch update feature if supported by the JDBC driver.
     * Returns Some(rowsAffected), or None if the database returned no row count for some part of the batch. If any part
     * of the batch fails, an exception is thrown.
     *
    ...
    /**
     * Insert multiple rows, including AutoInc columns. This is not supported by all database engines (see
     * [[slick.jdbc.JdbcCapabilities.forceInsert]]). Uses JDBC's batch update feature if supported by the JDBC driver.
     * Returns Some(rowsAffected), or None if the database returned no row count for some part of the batch. If any part
     * of the batch fails, an exception is thrown.
     */
    ...
    /**
     * Insert a single row if its primary key does not exist in the table, otherwise update the existing record. Note
     * that the return value will be None if an update was performed and Some if the operation was insert
     */
     ...
     * Returns Some(rowsAffected), or None if the database returned no row count for some part of the batch. If any part
     * of the batch fails, an exception is thrown. The option parameter specifies how the operation is to be
     * performed.(default is [[RowsPerStatement.All]]) Note unlike [[insertOrUpdate]], client-side emulation is not
     * supported.
     */
    def insertOrUpdateAll(
        values: Iterable[U],
        option: RowsPerStatement = defaultRowsPerStatement
  ...
  trait CountingInsertActionComposer[U] extends InsertActionComposer[U] {
    ...
    /**
     * Add a mapping from the inserted values and the generated key to compute a new return value. When using
     * [[insertAll]], some JDBC drivers may not be able to return the generated key. In that case, the collection of
     * keys returned by [[insertAll]] will be Nil.
     */
  ...
  trait ReturningInsertActionComposer[U, RU] extends InsertActionComposer[U] with IntoInsertActionComposer[U, RU] {
     ...
     * @param f
     *   Function that maps inserted values and generated keys to a desired value.
     * @tparam R
  ...
  protected abstract class InsertActionComposerImpl[U](val compiled: CompiledInsert) extends InsertActionComposer[U] {
    ...
    private def insertSingleRow(
        sql: Vector[String],
        ctx: JdbcBackend#JdbcActionContext,
        a: compiled.Artifacts,
        value: U
    ) =
    ...
    class MultiInsertAction(a: compiled.Artifacts, values: Iterable[U], rowsPerStatement: slick.jdbc.RowsPerStatement)
      ...
      protected def doUnbatched(ctx: JdbcBackend#JdbcActionContext, sql: Vector[String]) = {
        val results =
          for (value <- values.iterator) yield insertSingleRow(sql, ctx, a, value)
    ...
    class InsertOrUpdateAction(value: U)
      ...
      protected def emulate(value: U, checkSql: String, updateSql: String, insertSql: String)(implicit
          session: JdbcBackend#Session
      ): SingleInsertOrUpdateResult = {
  ...
  protected class ReturningInsertActionComposerImpl[U, QR, RU](
      compiled: CompiledInsert,
      val keys: Node,
      val mux: (U, QR) => RU
  ) extends InsertActionComposerImpl[U](compiled)
    ...
    protected def retMany(values: Iterable[U], individual: Seq[SingleInsertResult]): Seq[SingleInsertResult] =
      individual
