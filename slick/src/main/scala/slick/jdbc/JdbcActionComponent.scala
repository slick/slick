...
trait JdbcActionComponent extends SqlActionComponent {
  ...
  abstract class SimpleJdbcProfileAction[+R](_name: String, val statements: Vector[String]) extends SynchronousDatabaseAction[R, NoStream, JdbcBackend#JdbcActionContext, JdbcBackend#JdbcStreamingActionContext, Effect] with ProfileAction[R, NoStream, Effect] {
    self =>
  ...
  protected abstract class InsertActionComposerImpl[U](val compiled: CompiledInsert) extends InsertActionComposer[U] {
    ...
    class InsertOrUpdateAllAction(values: Iterable[U], rowsPerStatement: RowsPerStatement)
      extends MultiInsertAction(compiled.upsert, values)
