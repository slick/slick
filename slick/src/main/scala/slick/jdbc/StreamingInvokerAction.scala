...
/**
 * A streaming Action that wraps an Invoker. It is used for the Lifted Embedding, Direct Embedding, Plain SQL queries,
 * and JDBC metadata.
 */
trait StreamingInvokerAction[R, T, -E <: Effect]
    extends SynchronousDatabaseAction[R, Streaming[
      T
    ], JdbcBackend#JdbcActionContext, JdbcBackend#JdbcStreamingActionContext, E]
  ...
  private[this] class HeadOptionAction(val statements: Iterable[String])
      extends SynchronousDatabaseAction[Option[
        T
      ], NoStream, JdbcBackend#JdbcActionContext, JdbcBackend#JdbcStreamingActionContext, E]
