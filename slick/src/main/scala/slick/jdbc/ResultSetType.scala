...
object ResultSetType {
  ...
  /**
   * Represents a result set type that only allows result sets to be read sequentially (i.e. the cursor may only move
   * forward).
   */
  case object ForwardOnly extends ResultSetType(ResultSet.TYPE_FORWARD_ONLY)

  /**
   * Represents a result set type that allows result sets to be navigated in a non-linear way while keeping the original
   * data in the result set intact.
   */
  case object ScrollInsensitive extends ResultSetType(ResultSet.TYPE_SCROLL_INSENSITIVE)

  /**
   * Represents a result set type that allows result sets to be navigated in a non-linear way, and changes in the
   * underlying data to be observed.
   */
