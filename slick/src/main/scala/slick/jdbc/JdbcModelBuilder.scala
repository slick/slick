 ...
 * only useful if you need easy access to the JDBC metadata in order to influence how the model is generated. A good use
 * case would be interpreting column types or default values that Slick doesn't understand out of the box. If you just
 * want to remove or hard code some default values, transform the resulting model instead.
 *
 * The tight coupling can easily lead to source code incompatibilities in future versions. Avoid hooking in here if you
 * don't have to.
 *
 * @param ignoreInvalidDefaults
 *   see JdbcModelBuilder#ColumnBuilder#default
 */
class JdbcModelBuilder(mTables: Seq[MTable], ignoreInvalidDefaults: Boolean)(implicit ec: ExecutionContext)
    extends Logging {
  ...
  def readIndices(t: MTable): DBIO[Seq[Seq[MIndexInfo]]] = t.getIndexInfo().asTry.map {
    ...
    case Failure(e: java.sql.SQLException) => // TODO: this needs a test!
      logger
        .debug(s"Skipping indices of table ${t.name.name} due to exception during getIndexInfo: " + e.getMessage.trim)
      Seq()
  ...
  class TableBuilder(
    ...
    // models
    def buildModel(builders: Builders) = m
      .Table(namer.qualifiedName, columns, primaryKey, buildForeignKeys(builders), indices)
  ...
  class ColumnBuilder(tableBuilder: TableBuilder, meta: MColumn) {
    ...
    /**
     * Indicates whether a ColumnOption Primary key should be put into the model. Only valid for single column primary
     * keys.
     */
    ...
    /**
     * Indicates whether this should be a varchar in case of a string column. Should be based on the value of dbType in
     * the future.
     */
    def varying: Boolean = {
      import java.sql.Types.*
     ...
     * this method, logs the message and treats it as no default value for convenience.
     */
    def defaultColumnOption: Option[RelationalProfile.ColumnOption.Default[_]] = rawDefault
      .map(v => (v, tpe))
      .collect {
        case (v, _)
    ...
    private def convenientDefault: Option[RelationalProfile.ColumnOption.Default[_]] =
      try defaultColumnOption
        ...
        case _: java.lang.NumberFormatException if ignoreInvalidDefaults =>
  ...
  class PrimaryKeyBuilder(tableBuilder: TableBuilder, meta: Seq[MPrimaryKey]) {
    ...
    final def model: Option[m.PrimaryKey] = if (!enabled) None
    else
      Some(
