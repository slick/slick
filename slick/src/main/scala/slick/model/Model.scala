...
/**
 * A container class for Slick's data model The model can have circular references (e.g. a table has a foreign key which
 * points back to the table). The references are broken apart by using names instead of object references for back
 * references.
 */
case class Model(
    tables: Seq[Table],
  ...
  /**
   * Verifies consistency of the model by checking for duplicate names and references to non-existing entities. In case
   * such things are found, throws an AssertionError.
   */
  def assertConsistency(): Unit = {
    ...
    tables.foreach { table =>
      ...
      assert(
        ...
        "duplicate column names detected in table: " + table.name.table
      )
      ...
      assert(
        foreignKeys.count(_.name.isDefined) == foreignKeys.filter(_.name.isDefined).map(_.name).distinct.size,
      ...
      foreignKeys.foreach { fk =>
        ...
        fk.referencedColumns.foreach { pkColumn =>
          assert(
            ...
            msg("column " + pkColumn + " of table " + pkTable, "foreign key " + fk)
          )
        }
        fk.referencingColumns.foreach { fkColumn =>
          assert(
            fkTable.columns.contains(fkColumn),
            msg("column " + fkColumn + " of table " + fkTable, "foreign key " + fk)
          )
        }
      }
      assert(
        indices.count(_.name.isDefined) == indices.filter(_.name.isDefined).map(_.name).distinct.size,
