...
/**
 * For SQL back-ends which do not support real boolean types for fields and general expressions but which do have
 * special boolean expressions and operators, this phase injects conversions between fake and real boolean values.
 *
 * The default for booleans in the AST is to use the fake type (mapped to a numeric type by the profile). There are
 * specific places where a real boolean (that can be used in boolean expressions) is required or produced, so we inject
 * a call to ToRealBoolean or ToFakeBoolean as needed.
 */
class RewriteBooleans extends Phase {
  ...
  /**
   * Rewrite a single Node. This method can be overridden in subclasses to change the situations in which conversions
   * are applied.
   */
...
object RewriteBooleans {
  ...
  def rewriteFakeBooleanWithEquals(n: Node): Node =
    IfThenElse(
      ConstArray(Library.==.typed[Boolean](n, LiteralNode(1).infer()), LiteralNode(1).infer(), LiteralNode(0).infer())
    )
