...
/**
 * Create a ResultSetMapping root node, ensure that the top-level server-side node returns a collection, and hoist
 * client-side type conversions into the ResultSetMapping. The original result type (which was removed by
 * `removeMappedTypes`) is assigned back to the top level.
 */
        ...
        tpe match {
          case CollectionType(cons, el) =>
            ResultSetMapping(
              ...
              createResult(Ref(gen) :@ ch.nodeType.asCollectionType.elementType, el, syms)
            )
          case t =>
...
/**
 * Remove all mapped types from the tree and store the original top-level type as the phase state to be used later for
 * building the ResultSetMapping.
 */
