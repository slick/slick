...
/**
 * Rewrite monadic joins to applicative joins. After this phase all `Bind` nodes are of the form `Bind(_, _, Pure(_,
 * _))` (i.e. `flatMap` has been reduced to `map`).
 */
class RewriteJoins extends Phase {
  ...
  def tr(n: Node): Node = n.mapChildren(tr, keepType = true) match {
    case n @ Bind(s1, f1, Bind(_, Pure(StructNode(ConstArray()), _), select)) =>
    ...
    case n @ Bind(s1, f1, Bind(s2, j: Join, select)) =>
      ...
      val sel3 =
        if (m.isEmpty) sel2
          ...
          sel2.replace {
            case FwdPath(r1 :: rest) if r1 == sn && m.contains(rest) => m(rest)
    ...
    case n @ Bind(s1, Pure(f1, _), sel1) if !f1.isInstanceOf[Aggregate] =>
  ...
  def hoistFilters(j: Join): (Join, Set[TypeSymbol]) = {
    def hoist(ts: TermSymbol, n: Node): (Node, Option[Node], Set[TypeSymbol]) = (n match {
      ...
      case (Filter(s, f, p), invalid) =>
        val p2 = p.replace { case Ref(rs) :@ _ if rs == s => Ref(ts) }
  ...
  /**
   * Recursively hoist `Filter` out of of `Bind(_, Filter, Pure(StructNode))`. Returns the possibly modified tree plus a
   * set of invalidated TypeSymbols (non-empty if additional columns have to be added to the base projection for the
   * filters).
   */
  def hoistFilterFromBind(b: Bind): (Node, Set[TypeSymbol]) =
    (b.from match {
      ...
      case (Filter(fs1, from1, pred1), tss1) =>
        ...
        val sRefs = pred1
          .collect({ case p @ FwdPath(s :: rest) if s == fs1 => (p, FwdPath(b.generator :: rest)) }, stopOnMatch = true)
        val Bind(_, _, Pure(StructNode(struct1), pts)) = b
        val foundRefs = sRefs.map { case (p, pOnBGen) =>
          (p, (pOnBGen, /*None: Option[Symbol]*/ struct1.find { case (_, n) => pOnBGen == n }.map(_._1)))
        }.toMap
        ...
        val allRefs = foundRefs.collect { case (p, (_, Some(s))) => (p, s) } ++ newDefs.map { case (p, (_, s)) =>
          (p, s)
        }
        ...
        val (sel, tss) =
          if (newDefs.isEmpty) (b.select, tss1)
            ...
            (
              Pure(StructNode(struct1 ++ ConstArray.from(newDefs.map { case (_, (pOnGen, s)) => (s, pOnGen) })), pts),
              tss1 + pts
            )
        ...
        val pred = pred1.replace { case p: Select =>
          allRefs
            .get(p)
            .map(s => Select(Ref(fs) :@ b.nodeType.asCollectionType.elementType, s) :@ p.nodeType)
            .getOrElse(p)
        }
  ...
  /**
   * Recursively push refs from the right-hand side of a Join to the left-hand side out of the join. This is only
   * possible when they occur in a a mapping `Bind(_, _, Pure(StructNode))` directly at the RHS of a Join. Returns the
   * (possibly transformed) Join and replacements for forward paths into it.
   *
  ...
  def eliminateIllegalRefs(
      ...
      outsideRef: TermSymbol
  ): (Join, Map[List[TermSymbol], Node]) = {
    ...
    def trChild(n: Node, illegal: Set[TermSymbol], outsideRef: TermSymbol): (Node, Map[List[TermSymbol], Node]) =
      n match {
        case jch: Join => eliminateIllegalRefs(jch, illegal, outsideRef)
        case b @ Bind(s1, _, Pure(sn1 @ StructNode(_), ts)) =>
    ...
    if (l1m.isEmpty && r1m.isEmpty) (j, Map.empty)
      ...
      val on1 = j.on
        .replace(
          {
            case FwdPath(r1 :: rest) if r1 == j.leftGen && l1m.contains(rest)  => l1m(rest)
            case FwdPath(r1 :: rest) if r1 == j.rightGen && r1m.contains(rest) => r1m(rest)
          },
          keepType = true,
          bottomUp = true
        )
          ...
          v.replace(
            {
              case Ref(s) :@ tpe if s == j.leftGen =>
                Select(Ref(outsideRef) :@ j2.nodeType.asCollectionType.elementType, ElementSymbol(1)) :@ tpe
              case Ref(s) :@ tpe if s == j.rightGen =>
                Select(Ref(outsideRef) :@ j2.nodeType.asCollectionType.elementType, ElementSymbol(2)) :@ tpe
            },
            keepType = true
          )
        )
  ...
  /**
   * In a `Join(s1, _, _, Join(_, _, _, _, JoinType.Inner, on2), JoinType.Inner, on1)` where parts of `on2` refer to
   * `s1`, merge them into `on1`. Nested joins are processed recursively. The same is done in the opposite direction,
   * pushing predicates down into sub-joins if they only reference one side of the join.
   */
   ...
   * into a single Bind, provided that each element of either p1 or p2 contains not more than one path. This
   * transformation is not required for the correctness of join rewriting but it keeps the tree smaller to speed up
   * subsequent phases.
   */
  def flattenAliasingMap(b: Bind): Bind = b match {
    case Bind(s1, Bind(s2, f, Pure(StructNode(p1), _)), Pure(StructNode(p2), ts2)) =>
