...
class ExpandSums extends Phase {
  ...
  def expandSums(n: Node): Node = {
    ...
    def tr(tree: Node, oldDiscCandidates: Set[(TypeSymbol, List[TermSymbol])]): Node = {
      ...
      val tree3 = tree2 match {
        ...
        case OptionFold(from, LiteralNode(None) :@ OptionType(ScalaBaseType.nullType), oa @ OptionApply(Ref(s)), gen)
            if s == gen =>
        ...
        case OptionFold(from, ifEmpty, map, gen) =>
          ...
          val n2 = (ifEmpty, map) match {
            ...
            case _ =>
              ...
              else IfThenElse(ConstArray(Library.Not.typed[Boolean](pred), ifDefined, ifEmpty2))
          }
          n2.infer()
        ...
        case bind @ Bind(_, Join(_, _, _, _, jt, _), _)
  ...
  def translateJoin(bind: Bind, discCandidates: Set[(TypeSymbol, List[TermSymbol])]): Bind = {
    ...
    def findDisc(t: Type): Option[List[TermSymbol]] = {
      ...
      def find(t: Type, path: List[TermSymbol]): Vector[List[TermSymbol]] = t.structural match {
        ...
        case p: ProductType =>
          p.elements.iterator.zipWithIndex.flatMap { case (t, i) => find(t, ElementSymbol(i + 1) :: path) }.toVector
    ...
    def extend(side: Node, sym: TermSymbol, on: Node): (Node, Node, Boolean) = {
      ...
      val extend :@ CollectionType(_, extendedElementType) =
        Bind(extendGen, side, Pure(ProductNode(ConstArray(disc, Ref(extendGen))))).infer()
    ...
    def optionCast(idx: Int, createDisc: Boolean): Node = {
      ...
      val v = if (createDisc) {
        ...
        val disc = IfThenElse(
          ConstArray(
            Library.==.typed[Boolean](silentCast(OptionType(protoDisc.nodeType), protoDisc), LiteralNode(null)),
            DiscNone,
            Disc1
          )
        )
  ...
  def buildMultiColumnNone(tpe: Type): Node = (tpe.structural match {
    ...
    case OptionType(_)   => LiteralNode(tpe, None)
    case _               => throw new SlickException("Unexpected non-Option type in multi-column None")
  }) :@ tpe
  ...
  /**
   * Collect discriminator candidate fields in a predicate. These are all paths below an OptionApply, which indicates
   * their future use under a discriminator guard.
   */
  def collectDiscriminatorCandidates(n: Node): Set[(TypeSymbol, List[TermSymbol])] = n
  ...
  /**
   * Expand multi-column conditional expressions and SilentCasts. Single-column conditionals involving NULL values are
   * optimized away where possible.
   */
    ...
    def tr(n: Node): Node = n.mapChildren(tr, keepType = true) match {
      ...
      case cast @ Library.SilentCast(ch) :@ Type.Structural(ProductType(typeCh)) =>
        ...
        val elems = typeCh.zipWithIndex.map { case (t, idx) =>
          tr(Library.SilentCast.typed(t, ch.select(ElementSymbol(idx + 1))).infer())
        }
      ...
      case cond @ IfThenElse(clauses) if (clauses.last match {
            case LiteralNode(None) :@ OptionType(ScalaBaseType.nullType) => true; case _ => false
          }) =>
