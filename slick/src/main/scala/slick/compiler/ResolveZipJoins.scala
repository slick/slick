 ...
 *
 * We rely on having a Bind around every Join and both of its generators, which should have been generated by
 * Phase.forceOuterBinds. The inner Binds need to select `Pure(StructNode(...))` which should be the outcome of
...
class ResolveZipJoins(rownumStyle: Boolean = false) extends Phase {
  ...
  /**
   * Transform a `zipWithIndex` operation of the form `Bind(s1, Join(_, _, Bind(ls, from, Pure(StructNode(defs), _)),
   * RangeFrom(offset), JoinType.Zip, LiteralNode(true)), p)` into an equivalent mapping operation using `RowNum`. This
   * method can be overridden in subclasses to implement non-standard translations.
   */
  def transformZipWithIndex(
    ...
    val lbind = from match { // Ensure there is a Bind under the new Subquery, as required after forceOuterBinds
      ...
      case _ => // Other node -> First wrap in identity Bind, then in Subquery
  ...
  /**
   * Transform a `zip` operation of the form `Bind(s1, Join(jlsym, jrsym, l @ Bind(_, _, Pure(StructNode(ldefs), _)), r
   * \@ Bind(_, _, Pure(StructNode(rdefs), _)), JoinType.Zip, LiteralNode(true)), sel)` into an equivalent mapping
   * operation using `RowNum` by first transforming both sides of the join into `zipWithIndex` and then using
   * `transformZipWithIndex` on those.
   */
  def transformZip(
      s1: TermSymbol,
      jlsym: TermSymbol,
