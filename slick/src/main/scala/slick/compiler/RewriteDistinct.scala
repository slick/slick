...
class RewriteDistinct extends Phase {
  ...
  def apply(state: CompilerState) = if (state.get(Phase.assignUniqueSymbols).map(_.distinct).getOrElse(true))
    state.map(
      _.replace(
        {
        ...
        case Select(Ref(s), f) :@ tpe if s == s1 =>
          onFieldPos.get(f) match {
            ...
            case None =>
              ...
              Aggregate(as, Select(Ref(s), ElementSymbol(2)), Library.Min.typed(tpe, Select(Ref(as), f)))
          }
      }
