...
/**
 * Convert unreferenced StructNodes to single columns or ProductNodes (which is needed for aggregation functions and at
 * the top level).
 */
class RemoveFieldNames(val alwaysKeepSubqueryNames: Boolean = false) extends Phase {
  ...
  def apply(state: CompilerState) = state.map { n =>
    ClientSideOp.mapResultSetMapping(n, true) { rsm =>
      val CollectionType(_, NominalType(top, StructType(_))) = rsm.from.nodeType
        ...
        { n =>
          val refTSyms = n
            .collect[TypeSymbol] {
              ...
              case Comprehension(_, _ :@ CollectionType(_, NominalType(s, _)), _, _, _, _, _, _, _, _, _)
                  if alwaysKeepSubqueryNames =>
                s
            }
            .toSet
            ...
            case Pure(StructNode(ch), pts) if unrefTSyms contains pts =>
              val sel =
                if (ch.length == 1 && pts != top) ch(0)._2
                  ...
                  ProductNode(
                    ConstArray
                      .from(ch.map { case (s, n) => (requiredSyms.getOrElse(s, Int.MaxValue), n) }.toSeq.sortBy(_._1))
                      .map(_._2)
                  )
              (Pure(sel, pts), pts)
            case Pure(StructNode(ch), pts) if pts == top =>
              val sel =
                StructNode(
                  ConstArray
                    .from(
                      ch.map { case (s, n) => (requiredSyms.getOrElse(s, Int.MaxValue), (s, n)) }.toSeq.sortBy(_._1)
                    )
                    .map(_._2)
                )
              (Pure(sel, pts), pts)
