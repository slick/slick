...
/**
 * Optional phase which verifies that retyping the tree does not change any types. Useful for debugging type-related
  ...
  def check(tree: Node): Node = {
      ...
      .replace(
        {
          case t: TableNode =>
            t.nodeType match {
              case CollectionType(_, NominalType(_, _)) =>
              case _ =>
    ...
    if (errors.nonEmpty)
      throw new SlickTreeException(
        after
          .map(p => "After " + p.name + ": ")
          .getOrElse("") + errors.size + " type errors found in " + nodeCount + " nodes:",
        tree,
