...
/**
 * A custom compiler for INSERT statements. We could reuse the standard phases with a minor modification instead, but
 * this is much faster.
 */
class InsertCompiler(val mode: InsertCompiler.Mode) extends Phase {
  ...
  def apply(state: CompilerState) = state.map { tree =>
    ...
    def tr(n: Node): Node = n match {
      ...
      case Bind(gen, te @ TableExpansion(_, _: TableNode, _), Pure(sel, _)) =>
        setTable(te)
