              ...
              if (temp.isEmpty) n
                ...
                val from2 = sources.init
                  .foldRight(sources.last._2) { case ((_, n), z) =>
                    Join(new AnonSymbol, new AnonSymbol, n, z, JoinType.Inner, LiteralNode(true))
                  }
                  .infer()
                logger.debug("New 'from' with joined aggregates:", from2)
                val repl: Map[TermSymbol, List[TermSymbol]] = sources match {
                  case Vector((s, _)) => Map(s -> List(s1))
                  case _ =>
                    val len = sources.length
                    val _ = Iterator.iterate(s1)(_ => ElementSymbol(2))
...
object CreateAggregates extends Logging {
  ...
  def inlineMap(a: Aggregate): Aggregate = a.from match {
    case Bind(s1, f1, Pure(StructNode(defs1), _))
        if !f1.isInstanceOf[GroupBy] => // mergeToComprehensions always needs a Bind around a GroupBy
  ...
  /**
   * Find all scalar Aggregate calls in a sub-tree that do not refer to the given Symbol, and replace them by temporary
   * Refs.
   */
