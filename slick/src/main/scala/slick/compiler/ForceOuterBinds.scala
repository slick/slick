 ...
 * FilteredQueries are allowed on top of collection operations without a Bind in between, unless that operation is a
 * Join, Pure or Distinct node.
 */
class ForceOuterBinds extends Phase {
  ...
  def nowrap(n: Node): Node = n match {
    ...
    case f: FilteredQuery =>
      f.mapChildren { ch =>
        if ((ch eq f.from) && !(ch.isInstanceOf[Join] || ch.isInstanceOf[Distinct] || ch.isInstanceOf[Pure])) nowrap(ch)
        else maybewrap(ch)
      }
    case b: Bind =>
      b.mapChildren { ch =>
