...
class UnrollTailBinds extends Phase {
  ...
  def tr(n: Node): Node =
    n match {
      case bb @ Bind(br, downstream: Bind, Bind(bi, bf, select)) =>
        // make a new symbol
          ...
          node match {
            case Bind(bo, Filter(fa, ff1, where1), Filter(fb, ff2, where2)) =>
              val out = Bind(
                ...
                Bind(bm, Filter(fb, rep(ff2), rep(where2)), Bind(bi, rep(bf), rep(select)))
              )
              Some(out)
            case Bind(bo1, bi1: Bind, sel1) => replaceDownstream(bi1).map(replacement => Bind(bo1, replacement, sel1))
            case _                          => None
          }
