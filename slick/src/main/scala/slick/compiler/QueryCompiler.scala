...
class QueryCompiler(val phases: Vector[Phase]) extends Logging {
  ...
  /**
   * Return a new compiler with the new phase added directly after another phase (or a different implementation of the
   * same phase name).
   */
  ...
  /**
   * Return a new compiler with the new phase added directly before another phase (or a different implementation of the
   * same phase name).
   */
  ...
  /**
   * Return a new compiler that replaces an existing phase by a new one with the same name. The new phase must have a
   * State that is assignable to the original phase's state.
   */
  ...
  /**
   * Compile an AST in an existing `CompilerState`. This can be used for triggering compilation of subtrees within the
  ...
  /**
   * Compile an AST in an existing `CompilerState`, stopping just before the specified phase. This can be used for
   * triggering compilation of subtrees within the current `CompilerState`.
   */
...
/**
 * The current state of a compiler run, consisting of the current AST and additional immutable state of individual
 * phases. Mutability is confined to the SymbolNamer. The state is tied to a specific compiler instance so that phases
 * can call back into the compiler.
 */
