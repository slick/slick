...
/**
 * An optional phase which rewrites outer joins into more commonly supported operations for use on databases that lack
 * outer join support.
 *
 * Full outer joins are always emulated. Right joins can be replaced by left joins (or an emulated version thereof).
 * Left joins can be emulated with inner joins and unions.
 */
class EmulateOuterJoins(val useLeftJoin: Boolean, val useRightJoin: Boolean) extends Phase {
        ...
        Union(
          ...
          Bind(
            ...
            Filter(
              ...
              Library.Not
                .typed(on.nodeType, Library.Exists.typed(on.nodeType, Filter(rgen2, assignFreshSymbols(right), on2)))
            ),
            Pure(
              ProductNode(ConstArray(Ref(bgen), nullStructFor(right.nodeType.structural.asCollectionType.elementType)))
            )
          ),
          true
        ).infer()
        ...
        Union(
          ...
          Bind(
            ...
            Filter(
              ...
              Library.Not
                .typed(on.nodeType, Library.Exists.typed(on.nodeType, Filter(lgen2, assignFreshSymbols(left), on2)))
            ),
            Pure(
              ProductNode(ConstArray(nullStructFor(left.nodeType.structural.asCollectionType.elementType), Ref(bgen)))
            )
          ),
          true
        ).infer()
  ...
  def assignFreshSymbols(n: Node): Node = {
    ...
    n.replace(
      {
        case n: TableNode =>
          n.copy(identity = repl(n.identity).asInstanceOf[TableIdentitySymbol])(n.profileTable) :@ replaceTS(n.nodeType)
