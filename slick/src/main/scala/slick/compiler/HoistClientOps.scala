...
class HoistClientOps extends Phase {
  ...
  def apply(state: CompilerState) = state.map(ClientSideOp.mapResultSetMapping(_) { rsm =>
    ...
    from1 match {
      case Bind(s2, from2, Pure(StructNode(defs2), _)) =>
        // Extract client-side operations into ResultSetMapping
        val hoisted = defs2.map { case (ts, n) => (ts, n, unwrap(n, topLevel = true)) }
        logger.debug(
          "Hoisting operations from defs: " + hoisted.iterator.filter(t => t._2 ne t._3._1).map(_._1).mkString(", ")
        )
        val newDefsM = hoisted.iterator.zipWithIndex.map { case ((_, _, (n2, _)), idx) =>
          (idx, (n2, new AnonSymbol))
        }.toMap
        logger.debug("New defs: " + newDefsM)
        val oldDefsM = hoisted.iterator.zipWithIndex.map { case ((ts, _, (_, wrap)), idx) =>
          (ts, wrap(Select(Ref(rsm.generator), newDefsM(idx)._2)))
        }.toMap
        val bind2 = rewriteDBSide(
          Bind(s2, from2, Pure(StructNode(ConstArray.from(newDefsM.map(_._2.swap))), new AnonTypeSymbol)).infer()
        )
        val rsm2 = rsm
          .copy(
            from = bind2,
  ...
  def shuffle(n: Node): Node = n match {
    case n @ Bind(s1, from1, sel1) =>
      shuffle(from1) match {
        // Merge nested Binds
        case bind2 @ Bind(_, from2, Pure(StructNode(elems2), _)) if !from2.isInstanceOf[GroupBy] =>
        ...
        case from2 @ Join(
              sl1,
              sr1,
              bl @ Bind(_, _, Pure(StructNode(ldefs), _)),
              br @ Bind(_, _, Pure(StructNode(rdefs), _)),
              jt,
              on1
            ) if jt != JoinType.Outer =>
          ...
          val (bl2: Bind, lrepl: Map[TermSymbol, (Node => Node, AnonSymbol)] @unchecked) = if (jt != JoinType.Right) {
            ...
            val newDefsM = hoisted.iterator.map { case (_, _, (n2, _)) => (n2, new AnonSymbol) }.toMap
            logger.debug("New defs: " + newDefsM)
    ...
    case n @ CollectionCast(from1 :@ CollectionType(cons1, _), cons2) if !cons1.isUnique || cons2.isUnique =>
      shuffle(from1) match {
        case Bind(s1, bfrom1, sel1 @ Pure(StructNode(_), _)) if !bfrom1.isInstanceOf[GroupBy] =>
  ...
  /**
   * Remove a hoistable operation from a top-level column or join column and create a function to reapply it at an outer
   * layer.
   */
  def unwrap(n: Node, topLevel: Boolean): (Node, Node => Node) = n match {
    ...
    case IfThenElse(
          ConstArray(Library.==(ch, LiteralNode(null)), r1 @ LiteralNode(None), r2 @ LiteralNode(Some(1)))
        ) :@ OptionType(t) if t == ScalaBaseType.optionDiscType =>
      val (recCh, recTr) = unwrap(ch, topLevel)
