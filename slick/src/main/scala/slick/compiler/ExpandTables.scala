...
/**
 * Expand table-valued expressions in the result type to their star projection and compute the missing structural
 * expansions of table types. After this phase the AST should always be well-typed.
 */
class ExpandTables extends Phase {
  ...
  def apply(state: CompilerState) = {
    ...
    def createResult(expansions: collection.Map[TableIdentitySymbol, (TermSymbol, Node)], path: Node, tpe: Type): Node =
      tpe match {
        case p: ProductType =>
    ...
    val s2 = state
      .map { n =>
        ClientSideOp.mapServerSide(n) { tree =>
          ...
          val baseIdentity: Map[TypeSymbol, TypeSymbol] = tree
            .collect[(TypeSymbol, TypeSymbol)] { case TableNode(_, _, i, b) =>
              (i, b)
            }
            .toMap
          ...
          val structs: Map[TypeSymbol, Type] = tree
            .collect[(TypeSymbol, (FieldSymbol, Type))] { case s @ Select(_ :@ (n: NominalType), sym: FieldSymbol) =>
              baseIdentity(n.sourceNominalType.sym) -> (sym -> s.nodeType)
            }
            .toSeq
            .groupBy(_._1)
            .transform((_, v) => StructType(ConstArray.from(v.map(_._2).toMap)))
            .toMap
      ...
      .withWellTyped(true)
    if (createdOption)
      s2 + (Phase.assignUniqueSymbols -> state.get(Phase.assignUniqueSymbols).get.copy(nonPrimitiveOption = true))
    else s2
  }
}
