...
object RepShape extends Shape[FlatShapeLevel, Rep[_], Any, Rep[_]] {
  ...
  def encodeRef(value: Any, path: Node) = value.asInstanceOf[Rep[_]].encodeRef(path)
...
abstract class ProductNodeShape[Level <: ShapeLevel, C, M <: C, U <: C, P <: C] extends Shape[Level, M, U, P] {
  ...
  def pack(value: Any) = {
    val elems = shapes.iterator.zip(getIterator(value.asInstanceOf[M])).map{ case (p, f) => p.pack(f) }
  ...
  def encodeRef(value: Any, path: Node) = {
    val elems = shapes.iterator.zip(getIterator(value.asInstanceOf[M])).zipWithIndex.map {
      case ((p, x), pos) => p.encodeRef(x, Select(path, ElementSymbol(pos + 1)))
    }
    buildValue(elems.toIndexedSeq)
  }
  def toNode(value: Any): Node = ProductNode(ConstArray.from(shapes.iterator.zip(getIterator(value.asInstanceOf[M])).map {
    case (p, f) => p.toNode(f)
  }.to(Vector)))
...
abstract class MappedProductShape[Level <: ShapeLevel, C, M <: C, U <: C, P <: C] extends ProductNodeShape[Level, C, M, U, P] {
  override def toNode(value: Any) = TypeMapping(super.toNode(value), MappedScalaType.Mapper(toBase, toMapped, None), classTag)
