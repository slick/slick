...
/**
 * A type class that encodes the unpacking `Mixed => Unpacked` of a `Query[Mixed]` to its result element type `Unpacked`
 * and the packing to a fully packed type `Packed`, i.e. a type where everything which is not a transparent container is
 * wrapped in a `Column[_]`.
 *
 * =Example:=
...
abstract class Shape[Level <: ShapeLevel, -Mixed_, Unpacked_, Packed_] {
  ...
  /**
   * Build a packed representation containing QueryParameters that can extract data from the unpacked representation
   * later. This method is not available for shapes where Mixed and Unpacked are different types.
   */
  def buildParams(extract: Any => Unpacked): Packed

  /**
   * Encode a reference into a value of this Shape. This method may not be available for shapes where Mixed and Packed
   * are different types.
   */
...
trait ConstColumnShapeImplicits extends RepShapeImplicits {
  ...
  /**
   * A Shape for ConstColumns. It is identical to `columnShape` but it ensures that a `ConstColumn[T]` packs to itself,
   * not just to `Rep[T]`. This allows ConstColumns to be used as fully packed types when compiling query functions.
   */
...
trait RepShapeImplicits extends OptionShapeImplicits {
  ...
  @inline
  implicit def optionShape[M, U, P, Level <: ShapeLevel](implicit
      sh: Shape[? <: Level, Rep[M], U, Rep[P]]
...
trait OptionShapeImplicits {
  ...
  @inline
  implicit def anyOptionShape[M, U, P, Level <: ShapeLevel](implicit
      sh: Shape[? <: Level, M, U, P]
  ): Shape[Level, Rep[Option[M]], Option[U], Rep[Option[P]]] =
    RepShape.asInstanceOf[Shape[Level, Rep[Option[M]], Option[U], Rep[Option[P]]]]
}
...
abstract class ProductNodeShape[Level <: ShapeLevel, C, M <: C, U <: C, P <: C] extends Shape[Level, M, U, P] {
  ...
  def encodeRef(value: Any, path: Node) = {
    val elems = shapesWithValues(value.asInstanceOf[M]).zipWithIndex.map { case ((p, x), pos) =>
      p.encodeRef(x, Select(path, ElementSymbol(pos + 1)))
    }
    buildValue(elems.toIndexedSeq)
  }
  def toNode(value: Any): Node = ProductNode(
    ConstArray.from(
      shapesWithValues(value.asInstanceOf[M])
        .map { case (p, f) =>
          p.toNode(f)
        }
        .to(Vector)
...
/**
 * A generic case class shape that can be used to lift a case class of plain Scala types to a case class of lifted
 * types. This allows the type to be used as a record type (like tuples and HLists) in the Lifted Embedding.
 *
 * Example:
 *
 * {{{
...
class CaseClassShape[
    ...
    PlainTuple,
    PlainCaseClass <: P
](mapLifted: LiftedTuple => LiftedCaseClass, mapPlain: PlainTuple => PlainCaseClass)(implicit
...
/**
 * A generic Product class shape that can be used to lift a class of plain Scala types to a class of lifted types. This
 * allows the type to be used as a record type (like tuples and HLists) in the Lifted Embedding.
 *
 * This can help with mapping tables >22 columns to classes, especially when using code generation. This can be used for
 * Scala 2.11 case classes >22 fields.
 *
 * Example:
 *
 * {{{
...
/**
 * The level of a Shape, i.e. what kind of types it allows. Subtypes of this trait are used as a phantom type for Shape
 * resolution. There are no instances of any ShapeLevel.
 */
trait ShapeLevel
...
/**
 * A limited version of ShapedValue which can be constructed for every type that has a valid shape. We use it to enforce
 * that a table's * projection has a valid shape. A ProvenShape has itself a Shape so it can be used in place of the
 * value that it wraps for purposes of packing and unpacking.
 */
trait ProvenShape[U] {
  def value: Any
...
object ProvenShape {
  ...
  implicit def proveShapeOf[T, U](v: T)(implicit sh: Shape[? <: FlatShapeLevel, T, U, ?]): ProvenShape[U] =
    new ProvenShape[U] {
      ...
      def packedValue[R](implicit ev: Shape[? <: FlatShapeLevel, ?, U, R]): ShapedValue[R, U] =
        ShapedValue(
          sh.pack(value.asInstanceOf[sh.Mixed]).asInstanceOf[R],
          sh.packedShape.asInstanceOf[Shape[FlatShapeLevel, R, U, ?]]
        )
    }

  /** The Shape for a ProvenShape */
  implicit def provenShapeShape[T, P](implicit
      shape: Shape[? <: FlatShapeLevel, T, T, P]
