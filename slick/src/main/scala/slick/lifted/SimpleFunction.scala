...
/**
 * A SimpleLiteral is inserted verbatim into a SQL query string. For the purpose of handling it in the query compiler it
 * is assumed to be an expression of the specified type.
 */
...
object SimpleExpression {
  ...
  def nullary[R: TypedType](f: JdbcStatementBuilderComponent#QueryBuilder => Unit): Rep[R] = {
    val g = apply((_: Seq[Node], qb: JdbcStatementBuilderComponent#QueryBuilder) => f(qb))
    g.apply(Seq())
  }
