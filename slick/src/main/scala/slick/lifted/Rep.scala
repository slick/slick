 ...
 *
 * All column operations are added with extension methods that depend on the type inside the `Rep`. These are defined
 * in: <ul> <li>[[slick.lifted.AnyExtensionMethods]] and [[slick.lifted.ColumnExtensionMethods]] for columns of all
 * types</li> <li>[[slick.lifted.OptionColumnExtensionMethods]] for columns of all `Option` types</li>
 * <li>[[slick.lifted.BaseColumnExtensionMethods]] for columns of all non-`Option` types</li>
 * <li>[[slick.lifted.NumericColumnExtensionMethods]] for columns of numeric types (and Options thereof)</li>
 * <li>[[slick.lifted.BooleanColumnExtensionMethods]] for columns of `Boolean` / `Option[Boolean]`</li>
 * <li>[[slick.lifted.StringColumnExtensionMethods]] for columns of `String` / `Option[String]`</li>
 * <li>[[slick.lifted.ColumnOrdered]] for using columns in `sortBy` calls</li> </ul>
 *
...
/**
 * Represents `Rep[Option[T]]` in all cases where `T` is not a column base type. This special representation is
 * necessary so that a non-Option `Rep` value can be retrieved for encoding Option-based operations. This base value is
 * of type `T` if `T <: Rep[_]`, otherwise of type `Rep[T]`.
 */
