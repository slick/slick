 ...
 * `ForeignKeyQuery` objects instead.
 */
...
class ForeignKeyQuery[E <: AbstractTable[_], U](
  ...
  /**
   * Combine the constraints of this `ForeignKeyQuery` with another one with the same target table, leading to a single
   * instance of the target table which satisfies the constraints of both.
   */
