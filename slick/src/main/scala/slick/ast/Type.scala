...
trait Type extends Dumpable {
  ...
  /**
   * Apply a transformation to all type children and reconstruct this type with the new children, or return the original
   * object if no child is changed.
   */
...
/**
 * Represents a type constructor that can be usd for a collection-valued query. The relevant information for Slick is
 * whether the elements of the collection keep their insertion order (isSequential) and whether only distinct elements
...
object TypedCollectionTypeConstructor {
  ...
  implicit def forColl[C[X] <: Iterable[X]](implicit
      ...
      tag: ClassTag[C[Any]]
  ): TypedCollectionTypeConstructor[C] =
 ...
 *
 * Compiler phases which change types may keep their own representation of the structural view but must update the AST
 * at the end of the phase so that all NominalTypes with the same symbol have the same structural view.
 */
 ...
 *
 * This is used by QueryInterpreter and MemoryProfile. Values stored in HeapBackend columns are also expected to use
 * these types.
 *
 * All profiles should support the following types which are used internally by the lifted embedding and the query
 * compiler: Boolean, Char, Int, Long, Null, String.
 */
...
object ScalaBaseType {
  ...
  implicit val nullType: ScalaBaseType[Null] = {
    implicit val _: ClassTag[Null] = scala.reflect.ClassTag.Null; new ScalaBaseType[Null]
  }
