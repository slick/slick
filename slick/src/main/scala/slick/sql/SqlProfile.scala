...
trait SqlProfile
  ...
  trait DDL extends SchemaDescriptionDef { self =>
     ...
     *
     * Composition is such that given {{{A.ddl ++ B.ddl}}} the create phases will be run in FIFO order and the drop
     * phases will be run in LIFO order.
     */
...
object SqlProfile {
  ...
  object ColumnOption {
    ...
    /**
     * Type as expected by the DBMS, e.g. VARCHAR or VARCHAR(254). Note that Slick's model omits the optional length
     * ascription for string columns here and carries the length in the separate ColumnOption Length instead. A length
     * ascription for string column is allowed though and can be used in a Slick Table subclass to pass it to the DBMS.
...
trait SqlActionComponent extends RelationalActionComponent { this: SqlProfile =>
  ...
  type StreamingProfileAction[+R, +T, -E <: Effect] <: SqlStreamingAction[R, T, E] with ProfileAction[R, Streaming[
    T
  ], E]
}
