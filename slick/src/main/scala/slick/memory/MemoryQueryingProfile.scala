...
trait MemoryQueryingProfile extends BasicProfile { self: MemoryQueryingProfile =>
  ...
  class MemoryCodeGen
      extends CodeGen
      with ResultConverterCompiler[QueryInterpreter.ProductValue, ArrayBuffer[Any], Nothing] {
    ...
    override def compile(n: Node): ResultConverter[QueryInterpreter.ProductValue, ArrayBuffer[Any], Nothing, ?] =
      n match {
        ...
        case Library.SilentCast(sel @ Select(_, ElementSymbol(idx)) :@ OptionType(_)) :@ tpe
          ...
          val base =
            createColumnConverter(sel, idx, None)
              .asInstanceOf[ResultConverter[QueryInterpreter.ProductValue, ArrayBuffer[Any], Nothing, Option[Any]]]
  ...
  object ProductOfCommonPaths {
    def unapply(n: ProductNode): Option[(TermSymbol, Vector[List[TermSymbol]])] = if (n.children.isEmpty) None
    else
