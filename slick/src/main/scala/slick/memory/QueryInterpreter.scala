...
/**
 * A query interpreter for MemoryProfile and for client-side operations that need to be run as part of distributed
 * queries against multiple backends.
 *
 * It uses ScalaType, ProductValue/StructValue and plain Scala collections to represent data. Queries are expected to be
 * in the shape after running all the standard query compiler phases (but not the extra relational phases) and assigning
 * ScalaTypes everywhere.
 *
 * @param db
...
class QueryInterpreter(db: HeapBackend#Database, params: Any) extends Logging {
    ...
    val res = n match {
      ...
      case Join(leftGen, rightGen, left, right, JoinType.Left, by) =>
        val res = run(left).asInstanceOf[Coll].flatMap { l =>
          ...
          if (inner.headOption.isEmpty)
            Vector(new ProductValue(Vector(l, createNullRow(right.nodeType.asCollectionType.elementType))))
          else inner
        }
        scope.remove(leftGen)
        scope.remove(rightGen)
      ...
      case Join(leftGen, rightGen, left, right, JoinType.Right, by) =>
        val res = run(right).asInstanceOf[Coll].flatMap { r =>
          ...
          if (inner.headOption.isEmpty)
            Vector(new ProductValue(Vector(createNullRow(left.nodeType.asCollectionType.elementType), r)))
          else inner
        }
        scope.remove(leftGen)
        scope.remove(rightGen)
      ...
      case Library.Avg(ch) =>
        ...
        reduceOptionIt[(Int, Any)](it, opt, (1, _), { case ((ai, a), (bi, b)) => (ai + bi, num.plus(a, b)) }).map {
          case (count, sum) =>
  ...
  def evalFunction(sym: TermSymbol, args: Seq[(Type, Any)], retType: Type) = sym match {
    ...
    case Library.% =>
      args(0)._1.asInstanceOf[ScalaNumericType[Any]].numeric.asInstanceOf[Integral[Any]].rem(args(0)._2, args(1)._2)
    ...
    case Library.CountDistinct =>
      val CollectionType(_, elType) = args(0)._1: @unchecked
      val coll = args(0)._2.asInstanceOf[Coll]
      (elType match {
        case ProductType(_) => coll.toSeq.distinct
        case _              => coll.toSeq.filter(v => v != null && v != None).distinct
      }).size
    ...
    case Library.Like =>
      val pat =
        compileLikePattern(args(1)._2.toString, if (args.length > 2) Some(args(2)._2.toString.charAt(0)) else None)
    ...
    case Library.Repeat if args.size == 2 => args(0)._2.asInstanceOf[String] * args(1)._2.asInstanceOf[Int]
    case Library.Substring if args.size == 3 =>
      args(0)._2.asInstanceOf[String].substring(args(1)._2.asInstanceOf[Int], args(2)._2.asInstanceOf[Int])
    case Library.Replace =>
      args(0)._2.asInstanceOf[String].replace(args(1)._2.asInstanceOf[String], args(2)._2.asInstanceOf[String])
