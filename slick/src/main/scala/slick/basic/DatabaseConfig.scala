...
trait DatabaseConfig[P <: BasicProfile] {
  ...
  /**
   * Get the configured Database. It is instantiated lazily when this method is called for the first time, and must be
   * closed after use.
   */
...
object DatabaseConfig {
  ...
  /**
   * Load a Profile and database configuration through [[https://github.com/typesafehub/config Typesafe Config]].
   *
   * The following config parameters are available: <ul> <li>`profile` (String, required): The fully qualified name of a
   * class or object which implements the specified profile. If the name ends with `$` it is assumed to be an object
   * name, otherwise a class name.</li> <li>`db` (Config, optional): The configuration of a database for the profile's
   * backend. For profiles extending `JdbcProfile` (which always use `JdbcBackend`), see
   * `JdbcBackend.DatabaseFactory.forConfig` for parameters that should be defined inside of `db`.</li> </ul>
   *
   * Note: Slick 3.2 also supports the old `driver` parameter as an alternative to `profile`. Old profile names (e.g.
   * ``slick.driver.DerbyDriver$`` for ``slick.jdbc.DerbyProfile$``) are recognized and translated to the new names.
   * This feature is deprecated and will be removed in a future release.
   *
   * @param path
   *   The path in the configuration file for the database configuration (e.g. `foo.bar` would find a profile name at
   *   config key `foo.bar.profile`) or an empty string for the top level of the `Config` object.
   * @param config
   *   The `Config` object to read from. This defaults to the global app config (e.g. in `application.conf` at the root
   *   of the class path) if not specified.
   * @param classLoader
  ...
  def forConfig[P <: BasicProfile: ClassTag](
    ...
    new DatabaseConfig[P] {
      ...
      val _: P = untypedP.asInstanceOf[P]
  ...
  /**
   * Load a profile and database configuration from the specified URI. If only a fragment name is given, it is resolved
   * as a path in the global app config (e.g. in `application.conf` at the root of the class path), otherwise as a path
   * in the configuration located at the URI without the fragment, which must be a valid URL. Without a fragment, the
   * whole config object is used.
   */
