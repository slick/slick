...
/**
 * Backend for the basic database and session handling features. Concrete backends like `JdbcBackend` extend this type
 * and provide concrete types for `Database`, `DatabaseFactory` and `Session`.
 */
trait BasicBackend { self =>
  ...
  /**
   * Create a Database instance through [[https://github.com/typesafehub/config Typesafe Config]]. The supported config
   * keys are backend-specific. This method is used by `DatabaseConfig`.
   *
   * @param path
  ...
  trait BasicDatabaseDef extends Closeable { this: Database =>
    ...
    /**
     * Free all resources allocated by Slick for this Database. This is done asynchronously, so you need to wait for the
     * returned `Future` to complete in order to ensure that everything has been shut down.
     */
    def shutdown: Future[Unit] = Future(close())(ExecutionContext.fromExecutor(AsyncExecutor.shutdownExecutor))

    /**
     * Free all resources allocated by Slick for this Database, blocking the current thread until everything has been
     * shut down.
     *
     * Backend implementations which are based on a naturally blocking shutdown procedure can simply implement this
     * method and get `shutdown` as an asynchronous wrapper for free. If the underlying shutdown procedure is
     * asynchronous, you should implement `shutdown` instead and wrap it with `Await.result` in this method.
     */
    override def close(): Unit

    protected def prioritizedRunnable(
        priority: => Priority,
    ...
    /**
     * Create a `Publisher` for Reactive Streams which, when subscribed to, will run the specified `DBIOAction` and
     * return the result directly as a stream without buffering everything first. This method is only supported for
     * streaming actions.
     *
     * The Publisher itself is just a stub that holds a reference to the action and this Database. The action does not
     * actually start to run until the call to `onSubscribe` returns, after which the Subscriber is responsible for
     * reading the full response or cancelling the Subscription. The created Publisher can be reused to serve a multiple
     * Subscribers, each time triggering a new execution of the action.
     *
     * For the purpose of combinators such as `cleanup` which can run after a stream has been produced, cancellation of
     * a stream by the Subscriber is not considered an error. For example, there is no way for the Subscriber to cause a
     * rollback when streaming the results of `someQuery.result.transactionally`.
     *
     * When using a JDBC back-end, all `onNext` calls are done synchronously and the ResultSet row is not advanced
     * before `onNext` returns. This allows the Subscriber to access LOB pointers from within `onNext`. If streaming is
     * interrupted due to back-pressure signaling, the next row will be prefetched (in order to buffer the next result
     * page from the server when a page boundary has been reached).
     */
    ...
    protected[this] def createPublisher[T](
        a: DBIOAction[?, Streaming[T], Nothing],
        createCtx: Subscriber[? >: T] => StreamingContext
    ): DatabasePublisher[T] = { (s: Subscriber[? >: T]) =>
    ...
    /**
     * Run an Action in an existing DatabaseActionContext. This method can be overridden in subclasses to support new
     * DatabaseActions which cannot be expressed through SynchronousDatabaseAction.
     *
     * @param streaming
     *   Whether to return the result as a stream. In this case, the context must be a `StreamingDatabaseActionContext`
     *   and the Future result should be completed with `null` or failed after streaming has finished. This method
     *   should not call any `Subscriber` method other than `onNext`.
     */
    ...
    private[this] def runInContextSafe[R](
        a: DBIOAction[R, NoStream, Nothing],
        ctx: Context,
        streaming: Boolean,
        topLevel: Boolean,
    ...
    private[this] def runInContextInline[R](
        a: DBIOAction[R, NoStream, Nothing],
        ctx: Context,
        streaming: Boolean,
        topLevel: Boolean,
    ...
    protected[this] def runSynchronousDatabaseAction[R](
        a: SynchronousDatabaseAction[R, NoStream, Context, StreamingContext, ?],
        ctx: Context,
                    ...
                    val res =
                      try a.run(ctx)
                        ...
                        case NonFatal(ex) =>
                          releaseSession(
                            ctx,
                            { case NonFatal(ex) => actionLogger.warn("Exception while closing session", ex) }
                          )
                          throw ex
                      }
                    releaseSession(ctx)
    ...
    protected[this] def streamSynchronousDatabaseAction(
        a: SynchronousDatabaseAction[?, ? <: NoStream, Context, StreamingContext, ? <: Effect],
                  ...
                  while ({
                    try {
                      ...
                      if (state eq null) { // streaming finished and cleaned up
                        releaseSession(
                          ctx = ctx,
                          handleCloseError = { case NonFatal(ex) =>
                            streamLogger.warn("Exception while closing session", ex)
                          }
                        )
                        if (!ctx.isPinned) setConnectionReleased()
                        ctx.sync = 0
    ...
    /**
     * Return the default ExecutionContext for this Database which should be used for running SynchronousDatabaseActions
  ...
  trait BasicSessionDef extends Closeable {
    ...
    /**
     * Force an actual database session to be opened. Slick sessions are lazy, so you do not get a real database
     * connection until you need it or you call force() on the session.
     */
    def force(): Unit
  }
  ...
  trait BasicActionContext extends ActionContext {
    ...
    /**
     * Whether to run all operations on the current thread or schedule them normally on the appropriate
     * ExecutionContext. This is used by the blocking API.
     */
     ...
     * [[http://gee.cs.oswego.edu/dl/jmm/cookbook.html]]) when executing something in a synchronous action context. It
     * is read when entering the context and written when leaving so that all writes to non-volatile variables within
     * the context are visible to the next synchronous execution.
     */
    ...
    /**
     * Used for the sequence counter in Action debug output. This variable is volatile because it is only updated
     * sequentially but not protected by a synchronous action context.
     */
  ...
  class BasicStreamingActionContext(
     ...
     * Whenever this value drops to 0, streaming is suspended. When it is raised up from 0 in `request`, streaming is
     * scheduled to be restarted. It is initially set to `Long.MinValue` when streaming starts. Any negative value above
     * `Long.MinValue` indicates the actual demand at that point. It is reset to 0 when the initial streaming ends.
     */
    private[this] val remaining = new AtomicLong(Long.MinValue)

    /**
     * An error that will be signaled to the Subscriber when the stream is cancelled or terminated. This is used for
     * signaling demand overflow in `request()` while guaranteeing that the `onError` message does not overlap with an
     * active `onNext` call.
     */
    private[BasicBackend] var deferredError: Throwable = null

    /**
     * The state for a suspended streaming action. Must only be set from a synchronous action context.
     */
    ...
    private[BasicBackend] var streamingAction
        : SynchronousDatabaseAction[?, ? <: NoStream, Context, StreamingContext, ? <: Effect] = null
    ...
    /**
     * Indicate that the specified number of elements has been delivered. Returns the remaining demand. This is an
     * atomic operation. It must only be called from the synchronous action context which performs the streaming.
     */
    def delivered(num: Long): Long = remaining.addAndGet(-num)

    /**
     * Get the current demand that has not yet been marked as delivered and mark it as being in the current batch. When
     * this value is negative, the initial streaming action is still running and the real demand can be computed by
     * subtracting `Long.MinValue` from the returned value.
     */
    ...
    /**
     * Finish the stream with `onComplete` if it is not finished yet. May only be called from a synchronous action
     * context.
     */
