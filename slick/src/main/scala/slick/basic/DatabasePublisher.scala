...
abstract class DatabasePublisher[T] extends Publisher[T] { self =>
  ...
  /**
   * Create a Publisher that runs a synchronous mapping function on this Publisher. This can be used to materialize LOB
  ...
  /**
   * Consume the stream, processing each element sequentially on the specified ExecutionContext. The resulting Future
   * completes when all elements have been processed or an error was signaled.
   */
  def foreach[U](f: T => U)(implicit ec: ExecutionContext): Future[Unit] = {
    ...
    subscribe(new Subscriber[T] {
      ...
      def onNext(t: T): Unit = {
        lastMsg = Future(f(t))
        lastMsg.onComplete {
          case Success(_) => subscr.request(1L)
          case Failure(t) =>
            subscr.cancel()
            p.tryFailure(t)
        }
