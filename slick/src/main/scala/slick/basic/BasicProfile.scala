...
trait BasicProfile extends BasicActionComponent { self: BasicProfile =>
  ...
  /**
   * The capabilities supported by this profile. This can be used to query at runtime whether a specific feature is
   * supported.
   */
  ...
  /**
   * A schema description contains the SQL statements for creating and dropping database entities. Schema descriptions
   * can be combined for creating or dropping multiple entities together, even if they have circular dependencies.
   */
  ...
  trait BasicAPI extends Aliases with ExtensionMethodConversions {
    ...
    implicit final def anyToShapedValue[T, U](value: T)(implicit
    ...
    implicit def streamableAppliedCompiledFunctionActionExtensionMethods[R, RU, EU, C[_]](
  ...
  /**
   * The API for using the query language with a single import statement. This provides the profile's implicits, the
   * Database API and commonly used query language types and objects.
   */
  val api: BasicAPI
  ...
  /**
   * The type of a (partially) compiled AST for Insert operations. Unlike querying or deleting, inserts may require
   * different compilation results which should be computed lazily.
   */
  type CompiledInsert
  ...
  /**
   * Load the configuration for this profile. This can be overridden in user-defined subclasses to load different
   * configurations.
   *
   * The default implementation does a breadth-first search in the supertype hierarchy of the runtime class until it
   * finds a class or trait with a name matching "slick.[...]Profile" and then returns uses this name as a path in the
   * application config. If no configuration exists at this path, an empty Config object is returned.
   */
...
trait BasicActionComponent { self: BasicProfile =>
  ...
  type StreamingProfileAction[+R, +T, -E <: Effect] <: BasicStreamingAction[R, T, E] with ProfileAction[R, Streaming[
    T
  ], E]
...
trait BasicStreamingAction[+R, +T, -E <: Effect] extends BasicAction[R, Streaming[T], E] {
  ...
  /**
   * Create an Action that returns only the first value of this stream of data. The Action will fail if the stream is
   * empty. Only available on streaming Actions.
   */
  def head: ResultAction[T, NoStream, E]

  /**
   * Create an Action that returns only the first value of this stream of data as an `Option`. Only available on
   * streaming Actions.
   */
...
trait FixedBasicStreamingAction[+R, +T, -E <: Effect]
    ...
    with FixedBasicAction[R, Streaming[T], E]
