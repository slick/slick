...
/**
 * A Database I/O Action that can be executed on a database. The DBIOAction type allows a separation of execution logic
 * and resource usage management logic from composition logic. DBIOActions can be composed with methods such as
 * `andThen`, `andFinally` and `flatMap`. Individual parts of a composite DBIOAction are always executed serially on a
 * single database, but possibly in different database sessions, unless the session is pinned either explicitly (using
 * `withPinnedSession`) or implicitly (e.g. through a transaction).
 *
 * The actual implementation base type for all Actions is `DBIOAction`. `StreamingDBIO` and `DBIO` are type aliases
 * which discard the effect type (and the streaming result type in the latter case) to make DBIOAction types easier to
 * write when these features are not needed. All primitive DBIOActions and all DBIOActions produced by the standard
 * combinators in Slick have correct Effect types and are streaming (if possible).
 *
 * @tparam R
 *   The result type when executing the DBIOAction and fully materializing the result.
 * @tparam S
 *   An encoding of the result type for streaming results. If this action is capable of streaming, it is `Streaming[T]`
 *   for an element type `T`. For non-streaming DBIOActions it is `NoStream`.
 * @tparam E
 *   The DBIOAction's effect type, e.g. `Effect.Read with Effect.Write`. When composing actions, the correct combined
 *   effect type will be inferred. Effects can be used in user code, e.g. to automatically direct all read-only Actions
 *   to a slave database and write Actions to the master copy.
 */
sealed trait DBIOAction[+R, +S <: NoStream, -E <: Effect] extends Dumpable {

  /**
   * Transform the result of a successful execution of this action. If this action fails, the resulting action also
   * fails.
   */
  ...
  /**
   * Use the result produced by the successful execution of this action to compute and then run the next action in
   * sequence. The resulting action fails if either this action, the computation, or the computed action fails.
   */
  def flatMap[R2, S2 <: NoStream, E2 <: Effect](f: R => DBIOAction[R2, S2, E2])(implicit
      executor: ExecutionContext
  ): DBIOAction[R2, S2, E with E2] =
   ...
   */
  def flatten[R2, S2 <: NoStream, E2 <: Effect](implicit ev: R <:< DBIOAction[R2, S2, E2]) =
    flatMap(ev)(DBIO.sameThreadExecutionContext)

  /**
   * Run another action after this action, if it completed successfully, and return the result of the second action. If
   * either of the two actions fails, the resulting action also fails.
   */
  ...
  /**
   * Run another action after this action, if it completed successfully, and return the result of both actions. If
   * either of the two actions fails, the resulting action also fails.
   */
  ...
  /**
   * Run another action after this action, if it completed successfully, and zip the result of both actions with a
   * function `f`, then create a new DBIOAction holding this result, If either of the two actions fails, the resulting
   * action also fails.
   */
  def zipWith[R2, E2 <: Effect, R3](
      a: DBIOAction[R2, NoStream, E2]
  )(f: (R, R2) => R3)(implicit executor: ExecutionContext): DBIOAction[R3, NoStream, E with E2] =
    SequenceAction[Any, ArrayBuffer[Any], E with E2](Vector(this, a)).map { r =>
   ...
   * If the first action fails, its failure is propagated, whether the second action fails or succeeds. If the first
   * action succeeds, a failure of the second action is propagated.
   */
  ...
  /**
   * Run another action after this action, whether it succeeds or fails, in order to clean up or transform an error
   * produced by this action. The clean-up action is computed from the failure of this action, wrapped in `Some`, or
   * `None` if this action succeeded.
   *
   * @param keepFailure
   *   If this action returns successfully, the resulting action also returns successfully unless the clean-up action
   *   fails. If this action fails and `keepFailure` is set to `true` (the default), the resulting action fails with the
   *   same error, no matter whether the clean-up action succeeds or fails. If `keepFailure` is set to `false`, an error
   *   from the clean-up action will override the error from this action.
   */
  def cleanUp[E2 <: Effect](f: Option[Throwable] => DBIOAction[_, NoStream, E2], keepFailure: Boolean = true)(implicit
      executor: ExecutionContext
  ): DBIOAction[R, S, E with E2] =
  ...
  /**
   * Filter the result of this action with the given predicate. If the predicate matches, the original result is
   * returned, otherwise the resulting action fails with a NoSuchElementException.
   */
  ...
  /**
   * Transform the result of a successful execution of this action, if the given partial function is defined at that
   * value, otherwise, the result DBIOAction will fail with a `NoSuchElementException`.
   *
  ...
  def collect[R2](pf: PartialFunction[R, R2])(implicit executor: ExecutionContext): DBIOAction[R2, NoStream, E] =
    map(r1 =>
      pf.applyOrElse(
        r1,
        (r: R) => throw new NoSuchElementException(s"DBIOAction.collect partial function is not defined at: $r")
      )
    )

  /**
   * Replace a result of a successful execution of this action with the unit value. If this action fails, the resulting
   * action also fails.
   *
  ...
  /**
   * Replace a result of a successful execution of this action with the given value. If this action fails, the resulting
   * action also fails.
   *
  ...
  /**
   * Return an action which contains the Throwable with which this action failed as its result. If this action
   * succeeded, the resulting action fails with a NoSuchElementException.
   */
  def failed: DBIOAction[Throwable, NoStream, E] = FailedAction[E](this)

  /**
   * Convert a successful result `v` of this action into a successful result `Success(v)` and a failure `t` into a
   * successful result `Failure(t)`. This is the most generic combinator that can be used for error recovery. If
   * possible, use [[andFinally]] or [[cleanUp]] instead, because those combinators, unlike `asTry`, support streaming.
   */
  def asTry: DBIOAction[Try[R], NoStream, E] = AsTryAction[R, E](this)

  /**
   * Use a pinned database session when running this action. If it is composed of multiple database actions, they will
   * all use the same session, even when sequenced with non-database actions. For non-composite or non-database actions,
   * this has no effect.
   */
...
object DBIOAction {
  ...
  private[this] def groupBySynchronicity[R, E <: Effect](
      in: IterableOnce[DBIOAction[R, NoStream, E]]
  ...
  def sequence[R, M[+_] <: IterableOnce[_], E <: Effect](
      in: M[DBIOAction[R, NoStream, E]]
  )(implicit cbf: Factory[R, M[R]]): DBIOAction[M[R], NoStream, E] = {
    implicit val ec = DBIO.sameThreadExecutionContext
    def sequenceGroupAsM(g: Vector[DBIOAction[R, NoStream, E]]): DBIOAction[M[R], NoStream, E] =
      if (g.head.isInstanceOf[SynchronousDatabaseAction[_, _, _, _, _]]) { // fuse synchronous group
        new SynchronousDatabaseAction.Fused[M[
          R
        ], NoStream, BasicBackend#BasicActionContext, BasicBackend#BasicStreamingActionContext, E] {
            ...
            g.foreach(a =>
              b += a
                .asInstanceOf[SynchronousDatabaseAction[
                  R,
                  NoStream,
                  BasicBackend#BasicActionContext,
                  BasicBackend#BasicStreamingActionContext,
                  E
                ]]
                .run(context)
            )
    ...
    def sequenceGroupAsSeq(g: Vector[DBIOAction[R, NoStream, E]]): DBIOAction[Seq[R], NoStream, E] =
      if (g.length == 1) {
        if (g.head.isInstanceOf[SynchronousDatabaseAction[_, _, _, _, _]]) { // fuse synchronous group
          new SynchronousDatabaseAction.Fused[Seq[
            R
          ], NoStream, BasicBackend#BasicActionContext, BasicBackend#BasicStreamingActionContext, E] {
    ...
    grouped.length match {
      ...
      case n =>
        grouped.foldLeft(DBIO.successful(cbf.newBuilder): DBIOAction[mutable.Builder[R, M[R]], NoStream, E]) {
          (ar, g) =>
            for {
              r <- ar
  ...
  /**
   * A simpler version of `sequence` that takes a number of DBIOActions with any return type as varargs and returns a
   * DBIOAction that performs the individual actions in sequence, returning `()` in the end.
   */
  def seq[E <: Effect](actions: DBIOAction[_, NoStream, E]*): DBIOAction[Unit, NoStream, E] = {
    ...
    def sequenceSync(g: Vector[DBIOAction[Any, NoStream, E]]): DBIOAction[Unit, NoStream, E] =
      new SynchronousDatabaseAction.Fused[
        ...
        def run(context: BasicBackend#BasicActionContext) =
          g.foreach(
            _.asInstanceOf[SynchronousDatabaseAction[
              Any,
              NoStream,
              BasicBackend#BasicActionContext,
              BasicBackend#BasicStreamingActionContext,
  ...
  def traverse[A, B, M[+X] <: IterableOnce[X]](
      in: M[A]
  )(f: A => DBIO[B])(implicit cbf: Factory[B, M[B]]): DBIO[M[B]] = {
    implicit val ec = DBIO.sameThreadExecutionContext
    in.iterator
  ...
  def fold[T, E <: Effect](actions: Seq[DBIOAction[T, NoStream, E]], zero: T)(
    ...
    actions.foldLeft[DBIOAction[T, NoStream, E]](DBIO.successful(zero)) { (za, va) =>
      za.flatMap(z => va.map(v => f(z, v)))
    }

  /** A DBIOAction that pins the current session */
  private[slick] object Pin
  ...
  /**
   * An ExecutionContext used internally for executing plumbing operations during DBIOAction composition.
   */
...
case class FailureAction(t: Throwable)
    extends SynchronousDatabaseAction[
      Nothing,
      NoStream,
      BasicBackend#BasicActionContext,
...
case class FlatMapAction[+R, +S <: NoStream, P, -E <: Effect](
    base: DBIOAction[P, NoStream, E],
    f: P => DBIOAction[R, S, E],
    executor: ExecutionContext
...
/**
 * A DBIOAction that represents a `seq` or `andThen` operation for sequencing in the DBIOAction monad. Unlike
 * `SequenceAction` it only keeps the last result.
 */
case class AndThenAction[R, +S <: NoStream, -E <: Effect](as: IndexedSeq[DBIOAction[Any, NoStream, E]])
    extends DBIOAction[R, S, E] {
  def getDumpInfo = DumpInfo("andThen", children = as.zipWithIndex.map { case (a, i) => (String.valueOf(i + 1), a) })

  override def andThen[R2, S2 <: NoStream, E2 <: Effect](a: DBIOAction[R2, S2, E2]): DBIOAction[R2, S2, E with E2] =
    a match {
...
case class SequenceAction[R, +R2, -E <: Effect](as: IndexedSeq[DBIOAction[R, NoStream, E]])(implicit
    val cbf: Factory[R, R2]
...
case class CleanUpAction[+R, +S <: NoStream, -E <: Effect](
    base: DBIOAction[R, S, E],
    f: Option[Throwable] => DBIOAction[_, NoStream, E],
    keepFailure: Boolean,
    executor: ExecutionContext
) extends DBIOAction[R, S, E] {
...
case class NamedAction[+R, +S <: NoStream, -E <: Effect](a: DBIOAction[R, S, E], name: String)
    extends DBIOAction[R, S, E] {
...
trait ActionContext {
  ...
  /**
   * Pin the current session. Multiple calls to `pin` may be nested. The same number of calls to `unpin` is required in
   * order to mark the session as not pinned anymore. A pinned session will not be released at the end of a primitive
   * database action. Instead, the same pinned session is passed to all subsequent actions until it is unpinned. Note
   * that pinning does not force an actual database connection to be opened. This still happens on demand. May only be
   * called from a synchronous action context.
   */
 ...
 * `BasicBackend.DatabaseDef.run` supports this kind of action out of the box through
 * `BasicBackend.DatabaseDef.runSynchronousDatabaseAction` so that `run` does not need to be extended if all primitive
 * database actions can be expressed in this way. These actions also implement construction-time fusion for the
 * `andFinally`, `andThen`, `asTry`, `failed`, `withPinnedSession` and `zip` operations.
 *
 * The execution engine ensures that an [[ActionContext]] is never used concurrently and that all state changes
 * performed by one invocation of a SynchronousDatabaseAction are visible to the next invocation of the same or a
 * different SynchronousDatabaseAction.
 */
trait SynchronousDatabaseAction[
    +R,
    +S <: NoStream,
    -C <: BasicBackend#BasicActionContext,
    -SC <: BasicBackend#BasicStreamingActionContext,
    -E <: Effect
] extends DatabaseAction[R, S, E] { self =>

  /**
   * The type used by this action for the state of a suspended stream. A call to `emitStream` produces such a state
   * which is then fed back into the next call.
   */
  ...
  /**
   * Run this action synchronously and emit results to the context. This methods may throw an Exception to indicate a
   * failure.
   *
   * @param limit
  ...
  /**
   * Dispose of a `StreamState` when a streaming action is cancelled. Whenever `emitStream` returns `null` or throws an
   * Exception, it needs to dispose of the state itself. This method will not be called in these cases.
   */
  def cancelStream(context: SC, state: StreamState): Unit = ()

  /**
   * Whether or not this action supports streaming results. An action with a `Streaming` result type must either support
   * streaming directly or have a [[nonFusedEquivalentAction]] which supports streaming. This flag is not used if the
   * Action has a `NoStream` result type.
   */
  ...
  override def zip[R2, E2 <: Effect](a: DBIOAction[R2, NoStream, E2]): DBIOAction[(R, R2), NoStream, E with E2] =
    a match {
      case a: SynchronousDatabaseAction[_, _, _, _, _] =>
        new SynchronousDatabaseAction.Fused[(R, R2), NoStream, C, SC, E with E2] {
  ...
  override def andFinally[E2 <: Effect](a: DBIOAction[_, NoStream, E2]): DBIOAction[R, S, E with E2] = a match {
    case a: SynchronousDatabaseAction[_, _, _, _, _] =>
      new SynchronousDatabaseAction.Fused[R, S, C, SC, E with E2] {
        def run(context: C): R = {
          val res =
            try self.run(context)
            catch {
              case NonFatal(ex) =>
                try a.asInstanceOf[SynchronousDatabaseAction[Any, NoStream, C, SC, E2]].run(context)
  ...
  override def failed: DBIOAction[Throwable, NoStream, E] =
    new SynchronousDatabaseAction.Fused[Throwable, NoStream, C, SC, E] {
  ...
  override def asTry: DBIOAction[Try[R], NoStream, E] =
    new SynchronousDatabaseAction.Fused[Try[R], NoStream, C, SC, E] {
...
object SynchronousDatabaseAction {
  ...
  trait Fused[
      ...
      SC <: BasicBackend#BasicStreamingActionContext,
      -E <: Effect
  ] extends SynchronousDatabaseAction[R, S, C, SC, E] {
    def getDumpInfo =
      DumpInfo(name = "SynchronousDatabaseAction.Fused", children = Vector(("non-fused", nonFusedEquivalentAction)))
    override def supportsStreaming: Boolean = false
  }

  class FusedAndThenAction[
      +R,
      +S <: NoStream,
      C <: BasicBackend#BasicActionContext,
      SC <: BasicBackend#BasicStreamingActionContext,
      -E <: Effect
  ](val as: IndexedSeq[SynchronousDatabaseAction[Any, S, C, SC, E]])
      extends Fused[R, S, C, SC, E] {
    def run(context: C): R = {
      var res: Any = null
   ...
   * `DBIO.sameThreadExecutionContext` and produce a `SynchronousDatabaseAction` in their evaluation function (where
   * applicable). This cannot be verified at fusion time, so a wrongly fused action can fail with a `ClassCastException`
   * during evaluation.
   */
  private[slick] def fuseUnsafe[R, S <: NoStream, E <: Effect](a: DBIOAction[R, S, E]): DBIOAction[R, S, E] =
    a match {
      case FlatMapAction(base: SynchronousDatabaseAction[_, _, _, _, _], f, ec)
        ...
        new SynchronousDatabaseAction.Fused[
          ...
          def run(context: BasicBackend#BasicActionContext): R = {
            val b = base
              ...
              .run(context)
            val a2 = f
              .asInstanceOf[Any => SynchronousDatabaseAction[
                R,
                S,
                BasicBackend#BasicActionContext,
                BasicBackend#BasicStreamingActionContext,
      ...
      case CleanUpAction(base: SynchronousDatabaseAction[_, _, _, _, _], f, keepFailure, ec)
          if ec eq DBIO.sameThreadExecutionContext =>
        new SynchronousDatabaseAction.Fused[
          R,
          S,
