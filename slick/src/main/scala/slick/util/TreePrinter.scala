  ...
  def print(n: Dumpable, out: PrintWriter = new PrintWriter(new OutputStreamWriter(System.out))): Unit = {
    def dump(baseValue: Dumpable, prefix1: String, prefix2: String, name: String, level: Int): Unit = {
      ...
      children.zipWithIndex.foreach { case ((name, value), idx) =>
        val (p1, p2) =
          if (idx == children.size - 1) (lastChildPrefix1, lastChildPrefix2) else (childPrefix1, childPrefix2)
  ...
  def apply(n: Dumpable, poss: List[Int]*): Dumpable = new Dumpable {
    def getDumpInfo = {
      ...
      if (poss.isEmpty) parent
        ...
        parent.copy(children = parent.children.zipWithIndex.map { case ((name, ch), idx) =>
