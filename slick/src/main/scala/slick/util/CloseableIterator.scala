...
/**
 * An Iterator with a `close` method to close the underlying data source. Implementers must close the data source when
...
object CloseableIterator {
  ...
  /**
   * Using some Closeable resource and a function to create a CloseableIterator from it, return a wrapped
   * CloseableIterator which closes the resource when itself gets closed. If the function terminates abnormally, the
   * resource is closed immediately.
   */
