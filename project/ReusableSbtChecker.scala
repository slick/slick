...
/**
 * Based on the default task value of `sdlc` but modified so a single project can easily run it on multiple outputs
 */
class ReusableSbtChecker(val scaladocDir: String, val scanDir: String, val linkBase: String, log: Logger)
    extends Checker {
  var ok = true
