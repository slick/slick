...
object TypeProviders {
      ...
      Seq(
        ...
        Test / packageSrc / mappings ++= {
          ...
          ((Test / managedSources).value
            .pair(Path.relativeTo((Test / sourceManaged).value) | Path.flat)) ++ // Add generated sources to sources JAR
  ...
  def typeProvidersTask = Def.task {
    ...
    val cachedFun = FileFunction
      .cached(s.cacheDirectory / "type-providers", inStyle = FilesInfo.lastModified, outStyle = FilesInfo.exists) {
        (in: Set[File]) =>
