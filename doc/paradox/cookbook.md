...
Distinguish SlickActions by Effect type at runtime
-------------------------------------------------

<h3>Problem</h3>

When using a replicated database with a primary (read/write) node and one or more replica (read-only) nodes, we have to 
maintain separate database handles and run each `SlickAction` on the appropriate database handle. If a `SlickAction`
containing a write goes to the replica, it will fail. If a read-only `SlickAction` goes to the primary, we lose the 
benefit of using a replicated database. In a large application, it's tedious to remember to use the right handle for
every `SlickAction`, and it's easy to mix them up.

Ideally we would have a single method like `run` that takes a `SlickAction` and determines whether to run it on the
primary or the secondary.

Slick provides a phantom `Effect` type parameter in each `SlickAction`, but it's not immediately obvious how to use at 
runtime. For example, we cannot pattern-match on the `Effect` type parameter, as this parameter is erased at runtime 
due to type erasure. 

<h3>Solution</h3>

This solution shows one way that we can introspect the `Effect` type in order to determine whether the `SlickAction`
...
is false. We implement a `run` method which takes a `SlickAction` and an implicit instance of the `EffectInfo.ReadOnly`
type class for the `SlickAction`'s `Effect` type. We summon the type class instance and check the `isReadOnly` member 
to determine whether the given `SlickAction` should go to the primary or to the secondary.
