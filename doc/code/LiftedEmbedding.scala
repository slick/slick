...
object LiftedEmbedding {
  def main(args: Array[String]) = {
    ...
    class Users(tag: Tag) extends Table[User](tag, "users") {
      ...
      def * = (id.?, first, last) <> ((User.apply _).tupled, User.unapply)
    }
      ...
      db.run(DBIO.seq(
        ...
        db.run(schema.create).flatMap { _ =>
          ...
          val r1 = {
            ...
            val q = coffees.map(_.price)
            val action = q.result
            val result: IO[Seq[Double]] = db.run(action)
          ...
          val r2 = {
            ...
            val action = q.delete
            val affectedRowsCount: IO[Int] = db.run(action)
            val sql = action.statements.head
            //#delete1
            affectedRowsCount
          }
        ...
        val insertSql = coffees.insertStatement
        ...
        db.run(DBIO.seq(
          ...
          db.run(users.schema.create >> userWithId).flatMap { userWithIdRes =>
        ...
        db.run(
          ...
          assert(result.isInstanceOf[java.sql.Date])
        }
      }.flatMap { _ =>
        {
          //#mappedtype
...

        {
          ...
          final class PairShape[Level <: ShapeLevel, M <: Pair[?, ?], U <: Pair[?, ?] : ClassTag, P <: Pair[?, ?]](
                                                                                                                    val shapes: Seq[Shape[? <: ShapeLevel, ?, ?, ?]])
            extends MappedScalaProductShape[Level, Pair[?, ?], M, U, P] {
          ...
          implicit def pairShape[Level <: ShapeLevel, M1, M2, U1, U2, P1, P2](
                                                                               implicit s1: Shape[? <: Level, M1, U1, P1], s2: Shape[? <: Level, M2, U2, P2]
          ...
          db.run(as.schema.create >> insertAction >> q2.result).flatMap { result =>
            assert(result == Vector(Pair(3, Pair(42, "bb")), Pair(2, Pair(42, "cc"))))
            ...
            //#case-class-shape
            db.run(bs.schema.create >> insertActions >> q3.result).flatMap { result =>
              assert(result == Vector(B(3, "bb"), B(2, "cc")))
              ...
              //#combining-shapes
              db.run(cs.schema.create >> insertActions2 >> q4.result).map { result =>
                assert(result == Vector(C(Pair(9, "z"), B(3, "bb")), C(Pair(8, "y"), B(2, "cc"))))
              }
            }
          }
        }
