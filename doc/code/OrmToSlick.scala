...
object OrmToSlick extends App {
  ...
  try {
    ...
    Await.result(db.run(setup), Duration.Inf) {
      ...
      val q = session.createCriteria(classOf[Person]).add(id in Array(2, 99, 17, 234))
      // #criteriaQuery
      ...
      // #slickQueryWithTypes
      val q =
        (people: Query[People, Person, Seq]).filter((p: People) => ((p.age: Rep[Int]) < 5 || p.age > 65): Rep[Boolean])
      // #slickQueryWithTypes
    } {
