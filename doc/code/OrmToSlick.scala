    ...
    cats.Database.resource(dc).use { db =>
      ...
      db.run(setup).flatMap { _ =>
        ...
        val r1 = {
          ...
          val addresses: IO[Seq[Address]] = db.run(addressesAction)
          //#slickExecution
          addresses
        }
        ...
        r1.flatMap { _ =>
          ...
          db.run(
...

            {
              ...
              val chrisWithAddress: IO[(Person, Address)] =
                db.run(chrisQuery.withAddress.result.head)
              val stefanWithAddress: IO[(Person, Address)] =
