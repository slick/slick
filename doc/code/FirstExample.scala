...
/**
 * A simple example that uses statically typed queries against an in-memory H2 database. The example data comes from
 * Oracle's JDBC tutorial at http://docs.oracle.com/javase/tutorial/jdbc/basics/tables.html.
 */
object FirstExample {
  def main(args: Array[String]): Unit = {
    ...
    try {
      ...
      // #create
      val resultFuture = setupFuture
        .flatMap { _ =>
          // #readall
          // Read all coffees and print them to the console
          println("Coffees:")
          db.run(coffees.result)
            .map(_.foreach { case (name, supID, price, sales, total) =>
        ...
        .flatMap { _ =>
          ...
          // #join
          db.run(q2.result).map(_.foreach(t => println("  " + t._1 + " supplied by " + t._2)))
