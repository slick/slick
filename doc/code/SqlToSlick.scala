      ...
      //#overrideSql
         */
        {
          ...
          val slick =
            ...
            people.flatMap(p => addresses.filter(a => p.addressId === a.id).map(a => (p.name, a.city))).result
          ...
          val slick =
            ...
            (people join addresses on (_.addressId === _.id)).map { case (p, a) => (p.name, a.city) }.result
          // #slickQueryExplicitJoin
          val (sqlRes, slickRes) = Await.result(db.run(sql zip slick), Duration.Inf)
          assert(sqlRes == slickRes)
          assert(sqlRes.size > 0)
          ...
          val slick =
            // #slickQueryLeftJoin
            (addresses joinLeft people on (_.id === _.addressId)).map { case (a, p) => (p.map(_.name), a.city) }.result
          // #slickQueryLeftJoin
          val (sqlRes, slickRes) = Await.result(db.run(sql zip slick), Duration.Inf)
          assert(sqlRes == slickRes)
          assert(sqlRes.size > 0)
          ...
          val slickUpdate =
            // #slickQueryUpdate
            people.filter(_.name === "M Odersky").map(p => (p.name, p.age)).update(("M. Odersky", 54321))
          // #slickQueryUpdate
          val slickDelete =
            // #slickQueryDelete
