...
import com.novocode.squery.combinator.{Table, Join}
...
object Benchmark {
  ...
  def test1(print: Boolean) {
    ...
    val s1 = new QueryBuilder(q1).buildSelect
    val s2 = new QueryBuilder(q2).buildSelect
    val s3 = new QueryBuilder(q3).buildSelect
    val s4 = new QueryBuilder(q4).buildSelect
    val s5 = new QueryBuilder(q5).buildSelect
