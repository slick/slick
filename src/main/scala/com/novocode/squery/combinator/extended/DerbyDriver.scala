...
class DerbyDDLBuilder(table: AbstractBasicTable[_]) extends BasicDDLBuilder(table, DerbyDriver) {
  ...
  override protected def addTypeAndOptions(c: NamedColumn[_], sb: StringBuilder) {
    ...
    for(o <- c.options) o match {
      ...
      case BasicColumnOption.Default(v) => defaultLiteral = c.asInstanceOf[NamedColumn[Any]].typeMapper(DerbyDriver).valueToSQLLiteral(v)
    }
    if(sqlType eq null) sqlType = mapTypeName(c.typeMapper(DerbyDriver))
    sb append sqlType
