...
object SimpleFunction {
  def apply[T](fname: String)(implicit tm: TypeMapper[_,T]): (Seq[Column[_]] => OperatorColumn[T] with SimpleFunction) =
...
object SimpleBinaryOperator {
  def apply[T](fname: String)(implicit tm: TypeMapper[_,T]): ((Column[_], Column[_]) => OperatorColumn[T] with SimpleBinaryOperator) =
