...
class QueryInterpreter(db: HeapBackend#Database, params: Any) extends Logging {
  ...
  def evalFunction(sym: Symbol, args: Seq[(Type, Any)], retType: Type) = sym match {
    ...
    case Library.User => ""
    case Library.Substring if args.size == 2 =>  args(0)._2.asInstanceOf[String].substring(args(1)._2.asInstanceOf[Int] - 1)
    case Library.Substring if args.size == 3 => args(0)._2.asInstanceOf[String].
      substring(args(1)._2.asInstanceOf[Int] - 1, args(2)._2.asInstanceOf[Int] + 1)
    ...
    case Library.Take=> args(0)._2.asInstanceOf[String].take(args(1)._2.asInstanceOf[Int])
  }
