...
class JdbcModelBuilder(mTables: Seq[MTableExtended], ignoreInvalidDefaults: Boolean) extends Logging {
  ...
  class Table(val meta: MTableExtended){
    ...
    final lazy val columns: Seq[m.Column]
      = mColumns.map(c => Column(c).createModel)
    ...
    class Column(val meta: MColumn){
      ...
      def createModel = m.Column(
        ...
        options =
          ...
          (if(autoInc) Some(slick.ast.ColumnOption.AutoInc) else None) ++
          (if(createPrimaryKeyColumnOption) Some(slick.ast.ColumnOption.PrimaryKey) else None) ++
    ...
    class PrimaryKey(meta: Seq[MPrimaryKey]){
      ...
      final def createModel: Option[m.PrimaryKey] = if(!enabled) None else Some(
    ...
    class ForeignKey(meta: Seq[MForeignKey]){
      ...
      final def createModel: Option[m.ForeignKey] = if(!enabled) None else Some(m.ForeignKey(
        name,
        table.qualifiedName,
    ...
    class Index(meta: Seq[MIndexInfo]){
      ...
      final def createModel: Option[m.Index]
   ...
   */
  def createModel: slick.model.Model = {
    m.Model( mTables.sortBy(_.name.name).map(Table(_).createModel) )
  }
