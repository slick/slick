...
class ConvertToComprehensions extends Phase {
  ...
  def convert(n: Node): Node = convert1(n.nodeMapChildren(convert, keepType = true)) match {
    case c1 @ Comprehension(from1, where1, None, orderBy1,
      ...
      c2.copy(from = from1 ++ from2, where = where1 ++ where2,
        ...
        withClauseNodes = withClauseNodes
  ...
  def convert1(n: Node): Node = n match {
    ...
    case Bind(ogen, Comprehension(Seq((igen, from)), Nil, None, Nil, Some(Pure(isel, _)), None, None, Nil, Nil, Nil), Pure(osel, oident)) =>
    ...
    case b @ Bind(gen, gr @ GroupBy(fromGen, from, by, _), Pure(sel, _)) =>
      ...
      val newSel = sel.replace({
        case a @ Apply(fs, Seq(b@Comprehension(Seq((s1, Select(Ref(gen2), ElementSymbol(2)))), Nil, None, Nil, Some(Pure(pexpr, _)), None, None, Nil, Nil, Nil))) if gen2 == gen =>
  ...
  object TakeDrop {
    ...
    def unapply(n: Node): Option[(Node, Option[Node], Option[Node])] = n match {
      case Take(from, num) => unapply(from) match {
        ...
        case _ =>
          from match {
            case Comprehension(Seq((_, f)), Nil, None, Nil, None, Some(t), d, Nil, Nil, Nil) => Some((f, Some(constOp[Long]("min")(math.min)(t, num)), d))
            case Comprehension(Seq((_, f)), Nil, None, Nil, None, None, d, Nil, Nil, Nil) => Some((f, Some(num), d))
      ...
      case Drop(from, num) => unapply(from) match {
        ...
        case _ =>
          from match {
            case Comprehension(Seq((_, f)), Nil, None, Nil, None, Some(t), None, Nil, Nil, Nil) => Some((f, Some(constOp[Long]("max")(math.max)(LiteralNode(0L), constOp[Long]("-")(_ - _)(t, num))), Some(num)))
            case Comprehension(Seq((_, f)), Nil, None, Nil, None, None, Some(d), Nil, Nil, Nil) => Some((f, None, Some(constOp[Long]("+")(_ + _)(d, num))))
            case Comprehension(Seq((_, f)), Nil, None, Nil, None, Some(t), Some(d), Nil, Nil, Nil) => Some((f, Some(constOp[Long]("max")(math.max)(LiteralNode(0L), constOp[Long]("-")(_ - _)(t, num))), Some(constOp[Long]("+")(_ + _)(d, num))))
...
class FuseComprehensions extends Phase {
  ...
  def fuseComprehension(c: Comprehension): Comprehension = {
    ...
    val newWithClauseNodes = new ArrayBuffer[Node]
    ...
    c.from.foreach {
      case t @ (sym, from: Comprehension) if isFuseableInner(sym, from, prevSyms) =>
        ...
        if(isFuseable(c, from)) {
          ...
          for(n <- from.withClauseNodes) newWithClauseNodes += inline(n)
    ...
    if(fuse) {
      ...
      val c2 = Comprehension(
        ...
        newWithClauseNodes ++ c.withClauseNodes.map(inline)
      )
  ...
  def narrowStructure(n: Node): Node = n match {
    ...
    case Comprehension(from, _, _, _, None, _, _, _, _, _) => narrowStructure(from.head._2)
    case Comprehension(_, _, _, _, Some(n), _, _, _, _, _) => narrowStructure(n)
    case n => n
  }
  ...
  def createSelect(c: Comprehension): Comprehension = if(c.select.isDefined) c else {
    c.from.last match {
      case (sym, UnionLeft(Comprehension(_, _, _, _, Some(Pure(StructNode(struct), _)), _, _, _, _, _))) =>
        val r = Ref(sym)
  ...
  private def noParenthesesUnion(n: Comprehension): Comprehension = {
    ...
    def internalFuse(c: Comprehension): Comprehension = c match {
      case outerComprehension@Comprehension(from@Seq((innerComprehensionSymbol, innerComprehension: Comprehension)), where, groupBy, orderBy, select@Some(pure: Pure), fetch, offset, joinNodes, unionNodes, withClauseNodes)
        if (areThereOnlySelect(pure) :: where.isEmpty :: groupBy.isEmpty :: orderBy.isEmpty :: fetch.isEmpty :: offset.isEmpty :: joinNodes.isEmpty :: Nil).forall(_ == true) => {
        ...
        if (outerFieldSymbols != Nil && outerFieldSymbols == innerFieldSymbols) {
          ...
          innerComprehension.copy(select = Some(Pure(StructNode(newInnerColumns))), unionNodes = innerComprehension.unionNodes ++ unionNodes, withClauseNodes = innerComprehension.withClauseNodes ++ withClauseNodes)
        }
        else {
          outerComprehension
        }
    ...
    n match {
      case outerComprehension@Comprehension(from@Seq((innerUnionSymbol, innerUnion: Union)), _, _, _, _, _, _, _, _, _) => {
