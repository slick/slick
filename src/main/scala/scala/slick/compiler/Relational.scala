  ...
  object TakeDrop {
    ...
    def unapply(n: Node): Option[(Node, Option[Node], Option[Node])] = n match {
      case Take(from, num) => unapply(from) match {
        ...
        case _ =>
          from match {
            case Comprehension(Seq((_, f)), Nil, None, Nil, None, Some(t), d, Nil) => Some((f, Some(constOp[Long]("min")(math.min)(t, num)), d))
      ...
      case Drop(from, num) => unapply(from) match {
        ...
        case _ =>
          from match {
            case Comprehension(Seq((_, f)), Nil, None, Nil, None, Some(t), None, Nil) => Some((f, Some(constOp[Long]("max")(math.max)(LiteralNode(0L), constOp[Long]("-")(_ - _)(t, num))), Some(num)))
            case Comprehension(Seq((_, f)), Nil, None, Nil, None, None, Some(d), Nil) => Some((f, None, Some(constOp[Long]("+")(_ + _)(d, num))))
            case Comprehension(Seq((_, f)), Nil, None, Nil, None, Some(t), Some(d), Nil) => Some((f, Some(constOp[Long]("max")(math.max)(LiteralNode(0L), constOp[Long]("-")(_ - _)(t, num))), Some(constOp[Long]("+")(_ + _)(d, num))))
...
class FuseComprehensions extends Phase {
  ...
  private def noParenthesesJoin(n: Node): Node = {
    def testBasicConstraint(c: Comprehension) = c.where.isEmpty && c.groupBy.isEmpty && c.orderBy.isEmpty && c.fetch.isEmpty && c.offset.isEmpty
    ...
    n match {
      case outerComprehension@Comprehension(Seq((outerSymbol, Join(leftGen, rightGen, left: Comprehension, right: Comprehension, joinType, on))), outerWhere, outerGroupBy, outerOrderBy, outerSelect@Some(Pure(ProductNode(outerSelectNodes), _)), outerFetch, outerOffset, outerJoinNodes) if testEmptyJoinNodes(outerJoinNodes) => {
