    ...
    ClientSideOp.mapResultSetMapping(tree, false) { rsm =>
      ...
      val (rsmFrom, rsmProj) =
        if(t3 eq t2) {
          ...
          val Comprehension(_, _, _, _, Some(Pure(ProductNode(treeProjChildren), _)), _, _, _, _, _) = comp
          ...
          val ResultSetMapping(_, newFrom @ Comprehension(_, _, _, _, Some(Pure(StructNode(str), _)), _, _, _, _, _), newProj) = t3
