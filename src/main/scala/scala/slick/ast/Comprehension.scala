...
/** A SQL comprehension */
final case class Comprehension(from: Seq[(Symbol, Node)] = Seq.empty, where: Seq[Node] = Seq.empty, groupBy: Option[Node] = None, orderBy: Seq[(Node, Ordering)] = Seq.empty, select: Option[Node] = None, fetch: Option[Node] = None, offset: Option[Node] = None, joinNodes: Seq[Node] = Nil, unionNodes: Seq[Node] = Nil, withClauseNodes: Seq[Node] = Nil) extends DefNode {
  type Self = Comprehension
  val nodeChildren = from.map(_._2) ++ where ++ groupBy ++ orderBy.map(_._1) ++ select ++ fetch ++ offset ++ joinNodes ++ unionNodes ++ withClauseNodes
  override def nodeChildNames =
    ...
    withClauseNodes.zipWithIndex.map("withClauseNodes" + _._2)
  protected[this] def nodeRebuild(ch: IndexedSeq[Node]) = {
    ...
    val withClauseNodesOffset = unionNodesOffset + newUnionNodes.length
    val newWithClauseNodes = ch.slice(withClauseNodesOffset, withClauseNodesOffset + withClauseNodes.length)
    copy(
      ...
      withClauseNodes = newWithClauseNodes
    )
  }
  ...
  def nodeWithComputedType2(scope: SymbolScope, typeChildren: Boolean, retype: Boolean): Self = {
    ...
    val withClauseNodes2 = mapOrNone(withClauseNodes)(_.nodeWithComputedType(genScope, typeChildren, retype))
    ...
    if(same && newType == nodeType) this else {
      ...
      copy(
        ...
        withClauseNodes = withClauseNodes2.getOrElse(withClauseNodes)
      ).nodeTyped(newType)
    }
  }
}
