...
/** A SQL comprehension */
final case class Comprehension(from: Seq[(Symbol, Node)] = Seq.empty, where: Seq[Node] = Seq.empty, groupBy: Option[Node] = None, orderBy: Seq[(Node, Ordering)] = Seq.empty, select: Option[Node] = None, fetch: Option[Node] = None, offset: Option[Node] = None, unionNodes: Seq[Node] = Seq.empty) extends DefNode {
  type Self = Comprehension
  val nodeChildren = from.map(_._2) ++ where ++ groupBy ++ orderBy.map(_._1) ++ select ++ fetch ++ offset ++ unionNodes
  ...
  protected[this] def nodeRebuild(ch: IndexedSeq[Node]) = {
    ...
    val unionNodesOffset = offsetOffset + newOffset.length
