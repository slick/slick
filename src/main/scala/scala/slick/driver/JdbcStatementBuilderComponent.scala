...
trait JdbcStatementBuilderComponent { driver: JdbcDriver =>
  ...
  class QueryBuilder(val tree: Node, val state: CompilerState) { queryBuilder =>
    ...
    protected def buildJoinNodes(nodes: Seq[Node]): Unit = {
      nodes.foreach {
        case InternalJoinNode(symbol, right: Comprehension, on, jt, _, skipParenthesis) =>
          b" ${jt.sqlName} join "
          val resultSymbol =
            if (skipParenthesis) {
              b += quoteTableName(right.from.head._2.asInstanceOf[TableNode])
              right.from.head._1
            }
            else {
              b"("
              buildComprehension(toComprehension(right))
              b")"
              symbol
            }
          b += " " += symbolName(resultSymbol)
          on match {
            case LiteralNode(true) => b" on 1=1"
            case _ => b" on !$on"
          }
        case _ => throw new SlickException("Expected only InternalJoinNode.")
      }
    }
