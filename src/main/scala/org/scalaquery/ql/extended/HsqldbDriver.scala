...
class HsqldbDDLBuilder(table: AbstractBasicTable[_], profile: HsqldbDriver) extends BasicDDLBuilder(table, profile) {
  ...
  protected class HsqldbColumnDDLBuilder(column: RawNamedColumn) extends BasicColumnDDLBuilder(column) {
  ...
  override protected def createColumnDDLBuilder(c: RawNamedColumn) = new HsqldbColumnDDLBuilder(c)
