...
object RewriteGenerators {
  ...
  def rewriteGenerator(b: Node, gen: Symbol, from: Node): Node = {
    ...
    val (withNewSelect, replacementMap) = {
      val isTableBased = findSelect(from).isInstanceOf[AbstractTable[_]]
      ...
      } else (b.nodeMapChildren(n => if(n eq from) replaceSelect(from, struct, allSyms) else n), refsMap)
    }
    replaceReferences(withNewSelect, skip, allSyms, replacementMap)
  }
