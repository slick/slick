...
import org.scalaquery.ql.{TableNothing, AbstractTable, Column, TypeMapper, ConstColumn, ColumnBase}
import org.scalaquery.util.{ProductLinearizer, ValueLinearizer, NaturalTransformation2, RecordLinearizer}
...
abstract class Shape[-Mixed_, Unpacked_, Packed_] {
  ...
  protected[this] def impureShape =
    throw new SQueryException("Shape does not have the same Mixed and Unpacked type")
}
